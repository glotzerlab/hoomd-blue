...
class integrate_npt_tests (unittest.TestCase):
    ...
    def test(self):
        ...
        md.integrate.npt(all, T=1.2, tau=0.5, P=1.0, tauP=0.5);
        run(100);
    ...
    def test_mtk_orthorhombic(self):
        all = group.all();
        md.integrate.mode_standard(dt=0.005);
        md.integrate.npt(all, T=1.2, tau=0.5, P=1.0, tauP=0.5, couple="none");
        run(100);

    def test_mtk_tetragonal(self):
        all = group.all();
        md.integrate.mode_standard(dt=0.005);
        md.integrate.npt(all, T=1.2, tau=0.5, P=1.0, tauP=0.5, couple="xy");
        run(100);

    def test_mtk_triclinic(self):
        all = group.all();
        md.integrate.mode_standard(dt=0.005);
        md.integrate.npt(all, T=1.2, tau=0.5, P=1.0, tauP=0.5, couple="none", all=True);
        run(100);
    ...
    def test_set_params(self):
        md.integrate.mode_standard(dt=0.005);
        all = group.all();
        npt = md.integrate.npt(all, T=1.2, tau=0.5, P=1.0, tauP=0.5);
    ...
    def test_empty(self):
        ...
        #    nve = md.integrate.npt(group=empty, T=1.0, P=1.0, tau=0.5, tauP=0.5)
        #    run(1);
