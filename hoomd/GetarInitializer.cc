...
GetarInitializer::GetarInitializer(shared_ptr<const ExecutionConfiguration> exec_conf,
    ...
    if (m_traj.get())
        ...
        for (vector<Record>::const_iterator iter(allRecords.begin()); iter != allRecords.end();
             ++iter)
            {
    ...
    for (auto item : pyModes)
        ...
        for (unsigned int j(0); j < pybind11::len(pyKey); ++j)
            ...
            if (!insertRecord(name, key))
                {
                const string error_message(string("Can't find the requested property ") + name);
                m_exec_conf->msg->error() << error_message << endl;
                throw runtime_error(error_message);
                }
            else if (name == "any")
                {
                for (set<Record>::const_iterator iter(key.begin()); iter != key.end(); ++iter)
                    {
                    set<Record> single;
                    single.insert(*iter);
                    modes[single] = value;
...
shared_ptr<SystemSnapshot> GetarInitializer::restoreSnapshot(shared_ptr<SystemSnapshot>& systemSnap,
    ...
    for (map<set<Record>, string>::const_iterator iter(modes.begin()); iter != modes.end(); ++iter)
        {
        if ("any" == iter->second)
            {
            for (set<Record>::const_iterator setIter(iter->first.begin());
                 setIter != iter->first.end();
                 ++setIter)
                {
...
void GetarInitializer::fillSnapshot(shared_ptr<SystemSnapshot> snapshot)
    ...
    if (snapshot->particle_data.pos.size() != pdata_N)
        ...
        m_exec_conf->msg->notice(3)
            << "Filling particle positions with the value (0, 0, 0)" << endl;
    ...
    if (snapshot->particle_data.vel.size() != pdata_N)
        ...
        m_exec_conf->msg->notice(3)
            << "Filling particle velocities with the value (0, 0, 0)" << endl;
    ...
    if (snapshot->particle_data.accel.size() != pdata_N)
        ...
        m_exec_conf->msg->notice(3)
            << "Filling particle accelerations with the value (0, 0, 0)" << endl;
    ...
    if (snapshot->particle_data.orientation.size() != pdata_N)
        ...
        m_exec_conf->msg->notice(3)
            << "Filling particle orientations with the value (1, 0, 0, 0)" << endl;
        snapshot->particle_data.orientation
            = vector<quat<Scalar>>(pdata_N, quat<Scalar>(1, vec3<Scalar>(0, 0, 0)));
        }
    ...
    if (snapshot->particle_data.angmom.size() != pdata_N)
        ...
        m_exec_conf->msg->notice(3)
            << "Filling particle angular momenta with the value (0, 0, 0, 0)" << endl;
        snapshot->particle_data.angmom
            = vector<quat<Scalar>>(pdata_N, quat<Scalar>(0, vec3<Scalar>(0, 0, 0)));
        }
    ...
    if (snapshot->particle_data.inertia.size() != pdata_N)
        ...
        m_exec_conf->msg->notice(3)
            << "Filling particle moment of inertia with the value (1, 1, 1)" << endl;
    ...
    if (bond_N)
        ...
        for (unsigned int i((unsigned int)snapshot->bond_data.type_mapping.size());
             i < maxbondtype + 1;
             ++i)
            {
    ...
    if (pair_N)
        ...
        for (unsigned int i((unsigned int)snapshot->pair_data.type_mapping.size());
             i < maxpairtype + 1;
             ++i)
            {
    ...
    if (angle_N)
        ...
        for (unsigned int i((unsigned int)snapshot->angle_data.type_mapping.size());
             i < maxangletype + 1;
             ++i)
            {
    ...
    if (dihedral_N)
        ...
        for (unsigned int i((unsigned int)snapshot->dihedral_data.type_mapping.size());
             i < maxdihedraltype + 1;
             ++i)
            {
    ...
    if (improper_N)
        ...
        for (unsigned int i((unsigned int)snapshot->improper_data.type_mapping.size());
             i < maximpropertype + 1;
             ++i)
            {
...
void GetarInitializer::restoreSimultaneous(shared_ptr<SystemSnapshot> snapshot,
                                           const set<Record>& records,
                                           string frame)
    {
    set<string, IndexCompare> availableFrames;

    for (set<Record>::const_iterator iter(records.begin()); iter != records.end(); ++iter)
        {
    ...
    if (foundFrames.size() && ("latest" == frame || "any" == frame || "" == foundFrames.back()))
        ...
        if (!foundFrames.size())
            ...
            for (set<Record>::const_iterator iter(records.begin()); iter != records.end();)
                {
                msg << iter->getPath();
                if (++iter != records.end())
                    msg << ", ";
...
void GetarInitializer::restoreSingle(shared_ptr<SystemSnapshot> snap, const Record& rec)
    {
    bool found(false);

    // uint quantities
    if (rec.getName() == "dimensions" || rec.getName() == "type" || rec.getName() == "tag")
        {
        ...
        if (rec.getFormat() == UInt32)
            ...
            {
            const string error_message("Can't understand the format of the data at "
                                       + rec.getPath());
            m_exec_conf->msg->error() << error_message << endl;
            throw runtime_error(error_message);
            }
    ...
    // Scalar quantities
    else if (rec.getName() == "box" || rec.getName() == "mass" || rec.getName() == "charge"
             || rec.getName() == "diameter" || rec.getName() == "moment_inertia_tensor")
        {
        vector<Scalar> data;
...
vector<string> GetarInitializer::parseTypeNames(const string& json)
    ...
    for (string::const_iterator iter(json.begin()); iter != json.end(); ++iter)
        {
        if (*iter == '"')
            {
            if (save)
...
bool GetarInitializer::knownProperty(const Record& rec) const
    ...
    return (rec.getName() == "acceleration" || rec.getName() == "angular_momentum"
            || rec.getName() == "angular_momentum_quat" || rec.getName() == "body"
            || rec.getName() == "box" || rec.getName() == "center_of_mass"
            || rec.getName() == "charge" || rec.getName() == "diameter"
            || rec.getName() == "dimensions" || rec.getName() == "image" || rec.getName() == "mass"
            || rec.getName() == "moment_inertia" || rec.getName() == "moment_inertia_tensor"
            || rec.getName() == "orientation" || rec.getName() == "position"
            || rec.getName() == "tag" || rec.getName() == "type"
            || rec.getName() == "type_names.json" || rec.getName() == "velocity"
            || rec.getName() == "virial");
    }
...
bool GetarInitializer::insertRecord(const string& name, set<Record>& recs) const
    ...
    for (vector<Record>::const_iterator iter(m_knownRecords.begin()); iter != m_knownRecords.end();
         ++iter)
        {
