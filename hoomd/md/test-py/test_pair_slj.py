...
class pair_slj_tests (unittest.TestCase):
    ...
    def test(self):
        lj = md.pair.slj(r_cut=3.0, d_max=2.0);
    ...
    def test_nlist_subscribe(self):
        lj = md.pair.slj(r_cut=2.5, d_max=2.0);
    ...
    def test_nlist_subscribe(self):
        nl = md.nlist.cell()
        lj = md.pair.slj(r_cut=2.5, d_max=2.0, nlist=nl);
    ...
    def test_dmax(self):
        ...
        lj = md.pair.slj(r_cut=3.0);
        self.assertAlmostEqual(context.current.neighbor_list.cpp_nlist.getMaximumDiameter(), rmax,5)
