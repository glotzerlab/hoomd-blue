...
struct PotentialLinkData
    ...
    void clear()
        ...
        m_potential_intracluster_link_deltaU_fwds.clear();
        }

    void update(unsigned int i, unsigned int j, LongReal delta_u_ij_fwd)
        {
        m_potential_intracluster_link_idxs.push_back(std::make_pair(i, j));
        m_potential_intracluster_link_deltaU_fwds.push_back(delta_u_ij_fwd);
        }

    std::vector<std::pair<unsigned int, unsigned int>> m_potential_intracluster_link_idxs;
    std::vector<LongReal> m_potential_intracluster_link_deltaU_fwds;
    };
...
template<class Shape> class UpdaterVMMC : public Updater
    {
    public:
    //! Constructor
    ...
    */
    UpdaterVMMC(std::shared_ptr<SystemDefinition> sysdef,
    ...
    */
    ...
    unsigned int m_attempts_per_particle; //!< Number of attempted moves per particle each time
                                          //!< update() is called
...
template<class Shape> UpdaterVMMC<Shape>::~UpdaterVMMC()
    {
...
/*! Perform a cluster move
    ...
    2. Loop over all of i's neighbors (j):
        2a. Add j to the cluster with probability p_ij, which needs u_ij in the current state, and
   u_ij after only i has been moved 2b. If j added to cluster, loop over all of its neighbors and
   add to cluster with probability p_jk 2c. Continue recursively looping over neighbors until no
   candidate cluster members are left
    3. Do the move
    4. Do the reverse move virtually on particle i, and repeat the cluster-making process
        - This step gives you all of the information you need for evaluating the acceptance
   criterion
...
*/
template<class Shape> void UpdaterVMMC<Shape>::update(uint64_t timestep)
    {
    ...
    // get the AABB Tree
    hoomd::detail::AABBTree& mc_aabb_tree = m_mc->buildAABBTree();
    ...
    for (unsigned int i_trial = 0; i_trial < m_attempts_per_particle; i_trial++)
        ...
        for (unsigned int current_seed = 0; current_seed < m_pdata->getN(); current_seed++)
            ...
            LongReal deltaU = 0.0; // change in energy before and after the cluster move
            ...
            hoomd::RandomGenerator rng_i(
                hoomd::Seed(hoomd::RNGIdentifier::UpdaterVMMC, timestep, user_seed),
...
// linker must be an active particle before and after the move
            ...
            for (unsigned int potential_link_idx = 0;
                 potential_link_idx
                 < m_potential_link_data.m_potential_intracluster_link_idxs.size();
                 potential_link_idx++)
                {
                unsigned int linker_i
                    = m_potential_link_data.m_potential_intracluster_link_idxs[potential_link_idx]
                          .first; // recall: linker_i in cluster by construction
                unsigned int linkee_j
                    = m_potential_link_data.m_potential_intracluster_link_idxs[potential_link_idx]
                          .second;
                m_exec_conf->msg->notice(10) << "  Checking failed, intracluster links between "
                                             << linker_i << " and " << linkee_j << std::endl;
                if (m_cluster_data.m_linkers_added.find(linkee_j)
                    == m_cluster_data.m_linkers_added.end())
                    {
                    // j ultimately did not join the cluster, so there will in general be a
                    // contribution to deltaU for this i-j pair we have already accounted for the
                    // probability of not forming the i-j link, so all we do is add to deltaU
                    deltaU += m_potential_link_data
                                  .m_potential_intracluster_link_deltaU_fwds[potential_link_idx];
                    }
                else
                    {
                    m_exec_conf->msg->notice(5) << "    particle with tag " << linkee_j
                                                << " eventually joined the cluster." << std::endl;
                    // j ultimately joined the cluster so there will be no contribution to deltaU
                    // for this i-j pair we have also already accounted for the probability of not
                    // forming that link in the forward and reverse moves so there is nothing to do
                    // here except for explicitly add 0.0 to deltaU for the sake of code readability
                    deltaU += 0.0;
                    }
                }
            ...
            if (accept_cluster_move)
                {
                m_exec_conf->msg->notice(4) << "  VMMC move accepted, moving " << cluster_size
                                            << " particles as a cluster" << std::endl;
                if (move_type_translate)
                    {
                    m_count_total.translate_accept_count++;
                    m_count_total.translate_total_num_particles_in_moved_clusters
                        += (unsigned long long int)cluster_size;
                    }
                else
                    {
                    m_count_total.rotate_accept_count++;
                    m_count_total.rotate_total_num_particles_in_moved_clusters
                        += (unsigned long long int)cluster_size;
                    }
                ...
                {
                m_exec_conf->msg->notice(4) << "VMMC move rejected (p_acc = " << p_acc
                                            << " < r = " << r << ")" << std::endl;
                if (move_type_translate)
                    {
                    m_count_total.translate_reject_count++;
...
template<class Shape>
void export_UpdaterVirtualMoveMonteCarlo(pybind11::module& m, const std::string& name)
    {
    pybind11::class_<UpdaterVMMC<Shape>, Updater, std::shared_ptr<UpdaterVMMC<Shape>>>(m,
                                                                                       name.c_str())
        ...
        .def_property("translation_move_probability",
                      &UpdaterVMMC<Shape>::getTranslationMoveProbability,
                      &UpdaterVMMC<Shape>::setTranslationMoveProbability)
        .def_property("maximum_trial_rotation",
                      &UpdaterVMMC<Shape>::getMaximumTrialRotation,
                      &UpdaterVMMC<Shape>::setMaximumTrialRotation)
        .def_property("maximum_trial_translation",
                      &UpdaterVMMC<Shape>::getMaximumTrialTranslation,
                      &UpdaterVMMC<Shape>::setMaximumTrialTranslation)
        .def_property("attempts_per_particle",
                      &UpdaterVMMC<Shape>::getAttemptsPerParticle,
                      &UpdaterVMMC<Shape>::setAttemptsPerParticle)
        .def_property("maximum_trial_center_of_rotation_shift",
                      &UpdaterVMMC<Shape>::getMaximumTrialCenterOfRotationShift,
                      &UpdaterVMMC<Shape>::setMaximumTrialCenterOfRotationShift)
        .def_property("maximum_allowed_cluster_size",
                      &UpdaterVMMC<Shape>::getMaximumAllowedClusterSize,
                      &UpdaterVMMC<Shape>::setMaximumAllowedClusterSize)
        .def_property("cluster_size_limit_mode",
                      &UpdaterVMMC<Shape>::getClusterSizeLimitMode,
                      &UpdaterVMMC<Shape>::setClusterSizeLimitMode)
...
inline void export_hpmc_virtual_moves_counters(pybind11::module& m)
    ...
    pybind11::class_<hpmc_virtual_moves_counters_t>(m, "hpmc_virtual_moves_counters_t")
        .def_property_readonly("translate_counts",
                               &hpmc_virtual_moves_counters_t::getTranslateCounts)
        .def_property_readonly("translate_rejection_counts",
                               &hpmc_virtual_moves_counters_t::getTranslateRejectCounts)
        .def_property_readonly(
            "translate_num_particles_in_moved_clusters",
            &hpmc_virtual_moves_counters_t::getTotalNumParticlesInClustersTranslate)
