...
class IntegratorHPMCMonoImplicitNew : public IntegratorHPMCMono<Shape>
    ...
    protected:
        ...
        Scalar m_d_dep_negative;                                 //!< Depletant circumsphere diameeter for repulsive process
...
void IntegratorHPMCMonoImplicitNew< Shape >::update(unsigned int timestep)
    ...
    // List of particles whose circumspheres intersect particle i's excluded-volume circumsphere in old configuration
    std::vector<unsigned int> intersect_i;

    // List of particle images that intersect
    std::vector<unsigned int> image_i;

    // List of particles whose circumspheres intersect particle i's excluded-volume circumsphere in new configuration
    ...
    for (unsigned int i_nselect = 0; i_nselect < this->m_nselect; i_nselect++)
        ...
        for (unsigned int cur_particle = 0; cur_particle < this->m_pdata->getN(); cur_particle++)
                        ...
                        if (detail::overlap(this->m_aabb_tree.getNodeAABB(cur_node_idx), aabb))
                            ...
                            if (this->m_aabb_tree.isNodeLeaf(cur_node_idx))
                                ...
                                for (unsigned int cur_p = 0; cur_p < this->m_aabb_tree.getNodeNumParticles(cur_node_idx); cur_p++)
                                    ...
                                    if ((m_quermass || h_overlaps.data[this->m_overlap_idx(m_type,typ_j)]) && circumsphere_overlap)
                                        {
                ...
                for (unsigned int k = 0; k < intersect_i.size(); ++k)
                    ...
                    for (unsigned int l = 0; l < n; ++l)
                        ...
                        bool overlap_old = false;
                            ...
                            if (m_quermass || h_overlaps.data[this->m_overlap_idx(m_type, typ_i)])
                                {
                                n_overlap_checks++;
                                unsigned int err = 0;
            ...
            if (accept && m_type_negative != UINT_MAX)
                        ...
                        if (detail::overlap(this->m_aabb_tree.getNodeAABB(cur_node_idx), aabb))
                            ...
                            if (this->m_aabb_tree.isNodeLeaf(cur_node_idx))
                                ...
                                for (unsigned int cur_p = 0; cur_p < this->m_aabb_tree.getNodeNumParticles(cur_node_idx); cur_p++)
                                    ...
                                    if ((m_quermass || h_overlaps.data[this->m_overlap_idx(m_type_negative,typ_j)]) && circumsphere_overlap)
                                        {
                ...
                // for every pairwise intersection
                #ifdef ENABLE_TBB
                tbb::parallel_for((unsigned int)0, (unsigned int)intersect_i_new.size(), [&](unsigned int k)
                #else
                for (unsigned int k = 0; k < intersect_i_new.size(); ++k)
                    ...
                    unsigned int j = intersect_i_new[k];
                    ...
                    vec3<Scalar> rij(rj-ri - this->m_image_list[image_i_new[k]]);
                    Scalar d = sqrt(dot(rij,rij));
                    ...
                    for (unsigned int l = 0; l < n; ++l)
                        ...
                        if (!sphere)
                            ...
                            pos_test = cap_i ? generatePositionInSphericalCap(my_rng, ri, Ri, hi, rij)
                                ...
                                this->m_image_list[image_i_new[k]];
                            }
                        else
                            {
                            // generate a random position in the smaller sphere
                        ...
                        bool overlap_new = false;
                            ...
                            if (m_quermass || h_overlaps.data[this->m_overlap_idx(m_type_negative, typ_i)])
                                {
                                n_overlap_checks++;
                                unsigned int err = 0;
                                if (circumsphere_overlap && test_overlap(r_ij, shape_test, shape_i, err, m_sweep_radius))
                        ...
                        if (!m_quermass)
                            ...
                            if (h_overlaps.data[this->m_overlap_idx(m_type_negative, typ_i)])
                                {
                                n_overlap_checks++;
                                unsigned int err = 0;
                                if (circumsphere_overlap && test_overlap(r_ij, shape_test, shape_old, err, m_sweep_radius))
                        ...
                        for (unsigned int m = 0; m < intersect_i_new.size(); ++m)
                            {
                            // read in its position and orientation
                            unsigned int j = intersect_i_new[m];

                            vec3<Scalar> r_jk = ((i == j) ? pos_i : vec3<Scalar>(h_postype.data[j])) - pos_test -
