...
void UpdaterClusters<Shape>::findInteractions(unsigned int timestep, vec3<Scalar> pivot, quat<Scalar> q, bool swap,
    ...
    auto coordinate = m_pdata->getCoordinateType();
    ...
    if (coordinate == ParticleData::cartesian)
        {
        if (patch)
            {
   ...
   else if (coordinate == ParticleData::hyperspherical)
        {
        if (patch)
            {
            // test old configuration against itself
            ...
            for (unsigned int i = 0; i < m_n_particles_old; ++i)
                ...
                Scalar extent_i = 0.5*patch->getAdditiveCutoff(typ_i);
                OverlapReal R_query = std::max(0.0,r_cut_patch+extent_i-min_core_diameter/(OverlapReal)2.0);
                detail::AABB aabb_i = detail::AABB(hypersphere.hypersphericalToCartesian(quat_l_i,quat_r_i),R_query);
                    ...
                    if (detail::overlap(m_aabb_tree_old.getNodeAABB(cur_node_idx), aabb_i))
                        ...
                        if (m_aabb_tree_old.isNodeLeaf(cur_node_idx))
                            ...
                            for (unsigned int cur_p = 0; cur_p < m_aabb_tree_old.getNodeNumParticles(cur_node_idx); cur_p++)
                                ...
                                OverlapReal arc_length = detail::get_arclength_hypersphere(quat_l_i, quat_r_i, quat_l_j, quat_r_j, hypersphere);
        ...
        for (unsigned int i = 0; i < nptl; ++i)
                ...
                if (detail::overlap(m_aabb_tree_old.getNodeAABB(cur_node_idx), aabb_i))
                    ...
                    if (m_aabb_tree_old.isNodeLeaf(cur_node_idx))
                        ...
                        for (unsigned int cur_p = 0; cur_p < m_aabb_tree_old.getNodeNumParticles(cur_node_idx); cur_p++)
                            ...
                            if (check_circumsphere_overlap_hypersphere(shape_i, shape_j, hypersphere)
                                && h_overlaps.data[overlap_idx(typ_i,typ_j)]
                                && test_overlap_hypersphere(shape_i, shape_j, hypersphere, err))
                                {
                                // add connection
                                m_overlap.push_back(std::make_pair(h_tag.data[i],new_tag_j));
                                } // end if overlap
            ...
            if (patch)
                ...
                Scalar extent_i = 0.5*patch->getAdditiveCutoff(typ_i);
                OverlapReal R_query = std::max(0.0,r_cut_patch+extent_i-min_core_diameter/(OverlapReal)2.0);
                aabb_i = detail::AABB(hypersphere.hypersphericalToCartesian(shape_i.quat_l, shape_i.quat_r),R_query);
                    ...
                    if (detail::overlap(m_aabb_tree_old.getNodeAABB(cur_node_idx), aabb_i))
                        ...
                        if (m_aabb_tree_old.isNodeLeaf(cur_node_idx))
                            ...
                            for (unsigned int cur_p = 0; cur_p < m_aabb_tree_old.getNodeNumParticles(cur_node_idx); cur_p++)
                                ...
                                OverlapReal arc_length = detail::get_arclength_hypersphere(shape_i.quat_l, shape_i.quat_r,
    ...
    if (coordinate == ParticleData::cartesian && line && !swap)
        {
...
void UpdaterClusters<Shape>::update(unsigned int timestep)
    ...
    bool line = !swap && (m_mc->hasOrientation()
        || (coordinate == ParticleData::hyperspherical)
    ...
    else if (coordinate == ParticleData::hyperspherical)
	{
	    ...
	    theta = hoomd::UniformDistribution<Scalar>(Scalar(0.0),Scalar(2.0*M_PI))(rng);
            z = hoomd::UniformDistribution<Scalar>(Scalar(-1.0),Scalar(1.0))(rng);
	    ...
	    pr = quat<Scalar>(0,nr);
            ...
            throw std::runtime_error("point reflections on hypersphere not supported.\n");
            }
	}
