...
#include "HelfrichGeneralMeshForceComputeGPU.h"

using namespace std;

/*! \file HelfrichGeneralMeshForceComputeGPU.cc
    \brief Contains code for the HelfrichGeneralMeshForceComputeGPU class
*/
...
*/
HelfrichGeneralMeshForceComputeGPU::HelfrichGeneralMeshForceComputeGPU(std::shared_ptr<SystemDefinition> sysdef,
                                                         std::shared_ptr<MeshDefinition> meshdef)
    : HelfrichGeneralMeshForceCompute(sysdef, meshdef)
    {
    if (!m_exec_conf->isCUDAEnabled())
        {
        m_exec_conf->msg->error()
            << "Creating a HelfrichGeneralMeshForceComputeGPU with no GPU in the execution configuration"
            << endl;
        throw std::runtime_error("Error initializing HelfrichGeneralMeshForceComputeGPU");
        }
 ...
 */
void HelfrichGeneralMeshForceComputeGPU::computeForces(uint64_t timestep)
    {
    precomputeParameter();
    ...
    ArrayHandle<helfrich_param_t> d_params(m_params,
                                                             access_location::device,
                                                             access_mode::read);

    m_tuner_force->begin();
    kernel::gpu_compute_generalhelfrich_force(d_force.data,
                                       d_virial.data,
                                       m_virial.getPitch(),
                                       m_pdata->getN(),
                                       d_pos.data,
...
/*! Actually perform the force computation
    \param timestep Current time step
 */
void HelfrichGeneralMeshForceComputeGPU::precomputeParameter()
    {
    const GPUArray<typename MeshBond::members_t>& gpu_meshbond_list
        = this->m_mesh_data->getMeshBondData()->getGPUTable();
...
void export_HelfrichGeneralMeshForceComputeGPU(pybind11::module& m)
    {
    pybind11::class_<HelfrichGeneralMeshForceComputeGPU,
                     HelfrichGeneralMeshForceCompute,
                     std::shared_ptr<HelfrichGeneralMeshForceComputeGPU>>(m, "HelfrichGeneralMeshForceComputeGPU")
