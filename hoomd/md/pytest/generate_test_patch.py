...
if __name__ == '__main__':
    i = Particle(position = [0,0,0], global_to_particle_quat = from_axis_angle([0,0,1], 0))
    j = Particle(position = [1.2,0,0], global_to_particle_quat = from_axis_angle([0,0,1], np.deg2rad(-180 + 30)))

    # j.pointing

    lj = LJ(epsilon = 1, sigma = 1)
    patch = Patch(np.deg2rad(90), 20, [1,0,0], [1,0,0])

    pair = PatchPair(i, j, lj, patch)
    ...
    ykp = PatchPair(i, j, yk, patch)
    ykp.print_values()


    print(ykp.force(), yk.force(.2))


    ## Make plots varying pointing direction

    # Torque vs angular offset in xy plane
    v = []
    ...
    for position in [0.95, 0.99, 1, 2**(1/6), 1.2]:
        for angle in np.arange(0,90,10):
            
            j = Particle(position = [position,0,0], global_to_particle_quat = from_axis_angle([0,0,1], np.deg2rad(-angle + 180)))
            
            lj = LJ(epsilon = 1, sigma = 1)
            
            patch = Patch(np.deg2rad(alpha), omega, [1,0,0], [1,0,0])
    ...
    # i = Particle(position = [0,0,0], global_to_particle_quat = from_axis_angle([0,0,1], 0))
    # for position in [0.5, 1, 2**(1/6), 1.2]:
    #     #for angle in np.arange(0,90,10):
    #     angle = 85
    #     j = Particle(position = [position,0,0], global_to_particle_quat = from_axis_angle([0,0,1], np.deg2rad(angle + 180)))
    #     lj = LJ(epsilon = 1, sigma = 1)
