...
void dpd_conservative_force_test(std::shared_ptr<ExecutionConfiguration> exec_conf)
    ...
    std::shared_ptr<SystemDefinition> sysdef(
        new SystemDefinition(2, BoxDim(50.0), 1, 0, 0, 0, 0, exec_conf));
...
UP_TEST(DPD_ForceConservative_Test)
    {
    dpd_conservative_force_test<PotentialPair<EvaluatorPairDPDThermo>>(
...
UP_TEST(DPD_GPU_ForceConservative_Test)
    {
    dpd_conservative_force_test<
        PotentialPairGPU<EvaluatorPairDPDThermo, gpu_compute_dpdthermo_forces>>(
        std::shared_ptr<ExecutionConfiguration>(
            new ExecutionConfiguration(ExecutionConfiguration::GPU)));
    }
#endif

template<class PP_DPD> void dpd_temperature_test(std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    std::shared_ptr<SystemDefinition> sysdef(
        new SystemDefinition(1000, BoxDim(5.0), 1, 0, 0, 0, 0, exec_conf));
    std::shared_ptr<ParticleData> pdata = sysdef->getParticleData();
    std::shared_ptr<ParticleFilter> selector_all(
        new ParticleFilterTag(sysdef, 0, pdata->getN() - 1));
    std::shared_ptr<ParticleGroup> group_all(new ParticleGroup(sysdef, selector_all));
...
UP_TEST(DPD_Temp_Test)
    {
    dpd_temperature_test<PotentialPairDPDThermo<EvaluatorPairDPDThermo>>(
        std::shared_ptr<ExecutionConfiguration>(
            new ExecutionConfiguration(ExecutionConfiguration::CPU)));
    }
...
UP_TEST(DPD_GPU_Temp_Test)
    {
    dpd_temperature_test<
        PotentialPairDPDThermoGPU<EvaluatorPairDPDThermo, gpu_compute_dpdthermodpd_forces>>(
        std::shared_ptr<ExecutionConfiguration>(
            new ExecutionConfiguration(ExecutionConfiguration::GPU)));
    }
#endif
