...
namespace kernel
    {
// //! Kernel for calculating area of triangulated mesh on the GPU
// /*! \param d_area Device memory to write total surface area
//     \param N number of particles
//     \param d_pos device array of particle positions
//     \param box Box dimensions (in GPU format) to use for periodic boundary conditions
//     \param tlist List of mesh triangless stored on the GPU
//     \param n_triangles_list List of numbers of mesh triangles stored on the GPU
// */
// __global__ void gpu_compute_AreaConservation_area_kernel(Scalar* d_area,
...
// */
// hipError_t gpu_compute_AreaConservation_area( //Scalar* d_area,
//                                              const unsigned int N,
//                                              const Scalar4* d_pos,
//                                              const BoxDim& box,
//                                              const group_storage<6>* tlist,
...
//     hipFuncGetAttributes(&attr, (const void*)gpu_compute_AreaConservation_area_kernel);
...
__global__ void gpu_compute_AreaConservation_force_kernel(//Scalar* d_area,
    ...
    for (int triangle_idx = 0; triangle_idx < n_triangles; triangle_idx++)
        ...
        Fa = - K / (2 * At) * numerator_base * (- 1.0 * nab * rac * s_baac - nac * rab * s_baac + ds_dra * rab * rac);
        ...
        virial[1] += Scalar(1. / 2.) * da.y * Fa.x; // xy
        virial[2] += Scalar(1. / 2.) * da.z * Fa.x; // xz
        virial[3] += Scalar(1. / 2.) * da.y * Fa.y; // yy
        virial[4] += Scalar(1. / 2.) * da.z * Fa.y; // yz
        virial[5] += Scalar(1. / 2.) * da.z * Fa.z; // zz
        }
