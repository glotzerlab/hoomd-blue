...
def make_random(N, kT, seed):
    ...
    if hoomd.context.current.decomposition:
        pdata = _mpcd.MPCDParticleData(N, box, kT, seed, sysdef.getNDimensions(), hoomd.context.current.device.cpp_device, hoomd.context.current.decomposition.cpp_dd)
    else:
        pdata = _mpcd.MPCDParticleData(N, box, kT, seed, sysdef.getNDimensions(), hoomd.context.current.device.cpp_device)
