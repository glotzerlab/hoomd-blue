...
ConstraintEllipsoidGPU::ConstraintEllipsoidGPU(std::shared_ptr<SystemDefinition> sysdef,
    ...
    if (!m_exec_conf->isCUDAEnabled())
        ...
        m_exec_conf->msg->error()
            << "Creating a ConstraintEllipsoidGPU with no GPU in the execution configuration"
            << endl;
...
void export_ConstraintEllipsoidGPU(py::module& m)
    {
    py::class_<ConstraintEllipsoidGPU,
               ConstraintEllipsoid,
               std::shared_ptr<ConstraintEllipsoidGPU>>(m, "ConstraintEllipsoidGPU")
