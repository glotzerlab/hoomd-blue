...
class Communicator
    ...
    protected:
        ...
        // I moved this up to avoid warnings regarding initialization order while keeping all of my new variables in one place
        GPUVector<unsigned int> m_plan;          //!< Array of per-direction flags that determine the sending route

        /******************************
         * Starting Vyas's Code
         ******************************/
        // Start variables for exchangeGhosts
        ...
        // The below data are not actually stored as state data since that would double count particles, so instead of a main dataset and a copybuf there is a copybuf and a receive buffer
        ...
        // I don't think we actually care about these total numbers
        //unsigned int m_num_copy_ghosts_reverse[6];       //!< Total number of particles that are sent back to neighboring processors. The sum of m_num_forward_ghosts_reverse and m_num_copy_local_ghosts_reverse
        //unsigned int m_num_recv_ghosts_reverse[6];       //!< Number of reverse ghosts received per direction. Receive buffer corresponding to m_num_copy_ghosts_reverse
