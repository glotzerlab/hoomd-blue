...
class hpmc_gsd_shape_spec(unittest.TestCase):
    ...
    def setup_system(self, integrator, shape_params, expected_shapespec, filename, dim):
        if dim == 2:
        ...
        mc = integrator(seed=123);
        mc.shape_param.set('A', **shape_params["A"]);
        mc.shape_param.set('B', **shape_params["B"]);
        dumper = dump.gsd(filename=filename, group=group.all(), period=1, overwrite=True);
        dumper.dump_shape(mc);
        steps = 5
        hoomd.run(steps);
    ...
    def test_sphere(self):
        ...
        self.setup_system(integrator=hpmc.integrate.sphere, shape_params=shape_params, \
    ...
    def test_ellipsoid(self):
        ...
        self.setup_system(integrator=hpmc.integrate.ellipsoid, shape_params=shape_params, \
                          expected_shapespec=expected_shapespec, filename=self.tmp_file, dim=3);
    ...
    def test_convex_polyhedron(self):
        ...
        self.setup_system(integrator=hpmc.integrate.convex_polyhedron, shape_params=shape_params, \
                          expected_shapespec=expected_shapespec, filename=self.tmp_file, dim=3);
    ...
    def test_convex_spheropolyhedron(self):
        ...
        self.setup_system(integrator=hpmc.integrate.convex_spheropolyhedron, shape_params=shape_params, \
                          expected_shapespec=expected_shapespec, filename=self.tmp_file, dim=3);
    ...
    def test_general_polyhedron(self):
        ...
        self.setup_system(integrator=hpmc.integrate.polyhedron, shape_params=shape_params, \
                          expected_shapespec=expected_shapespec, filename=self.tmp_file, dim=3);
    ...
    def test_convex_polygon(self):
        ...
        self.setup_system(integrator=hpmc.integrate.convex_polygon, shape_params=shape_params, \
    ...
    def test_convex_spheropolygon(self):
        ...
        self.setup_system(integrator=hpmc.integrate.convex_spheropolygon, shape_params=shape_params, \
                          expected_shapespec=expected_shapespec, filename=self.tmp_file, dim=2);
    ...
    def test_simple_polygon(self):
        ...
        self.setup_system(integrator=hpmc.integrate.simple_polygon, shape_params=shape_params, \
                          expected_shapespec=expected_shapespec, filename=self.tmp_file, dim=2);
...
class dem_gsd_shape_spec(unittest.TestCase):

    def setup_system(self, pair_cls, shape_params, expected_shapespec, filename, dim):
        if dim == 2:
            system = hoomd.init.create_lattice(unitcell=hoomd.lattice.sq(a=2.50),n=5);
        elif dim == 3:
            system = hoomd.init.create_lattice(unitcell=hoomd.lattice.sc(a=2.50),n=5);
        ...
        pair = pair_cls(nlist=nl, radius=.5);
        pair.setParams('A', **shape_params["A"]);
        pair.setParams('B', **shape_params["B"]);
        md.integrate.nvt(group=group.all(), kT=1.0, tau=0.5)
        dumper = dump.gsd(filename=filename, group=group.all(), period=1, overwrite=True);
        dumper.dump_shape(pair);
        steps = 5
        hoomd.run(steps);
        reader = _hoomd.GSDReader(hoomd.context.exec_conf, filename, 0, False);
        for i in range(steps):
    ...
    def test_wca_2d(self):
        ...
        self.setup_system(pair_cls=dem.pair.WCA, shape_params=shape_params, \
                          expected_shapespec=expected_shapespec, filename=self.tmp_file, dim=2);
