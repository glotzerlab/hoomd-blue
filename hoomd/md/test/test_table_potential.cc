...
typedef std::function<std::shared_ptr<TablePotential>(std::shared_ptr<SystemDefinition> sysdef,
                                                      ...
                                                      unsigned int width)>
    table_potential_creator;

//! performs some really basic checks on the TablePotential class
void table_potential_basic_test(table_potential_creator table_creator,
                                std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    // perform a basic test to see of the potential and force can be interpolated between two
    // particles
...
void table_potential_type_test(table_potential_creator table_creator,
                               std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    // perform a basic test to see of the potential and force can be interpolated between two
    // particles
    std::shared_ptr<SystemDefinition> sysdef(
        new SystemDefinition(4, BoxDim(1000.0), 2, 0, 0, 0, 0, exec_conf));
...
UP_TEST(TablePotential_basic)
    ...
    table_potential_basic_test(table_creator_base,
...
UP_TEST(TablePotential_type)
    {
    table_potential_creator table_creator_base = bind(base_class_table_creator, _1, _2, _3);
    table_potential_type_test(table_creator_base,
                              std::shared_ptr<ExecutionConfiguration>(
                                  new ExecutionConfiguration(ExecutionConfiguration::CPU)));
    }
...
UP_TEST(TablePotentialGPU_basic)
    ...
    table_potential_basic_test(table_creator_gpu,
                               std::shared_ptr<ExecutionConfiguration>(
...
UP_TEST(TablePotentialGPU_type)
    {
    table_potential_creator table_creator_gpu = bind(gpu_table_creator, _1, _2, _3);
    table_potential_type_test(table_creator_gpu,
                              std::shared_ptr<ExecutionConfiguration>(
                                  new ExecutionConfiguration(ExecutionConfiguration::GPU)));
    }
#endif
