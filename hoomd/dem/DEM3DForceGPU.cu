...
__global__ void gpu_compute_dem3d_forces_kernel(const Scalar4* d_pos,
    ...
    do
        ...
        if (localThreadIdx + offset < numVerts)
            ...
            vertexConnectivity[localThreadIdx + offset]
                = d_vertexConnectivity[localThreadIdx + offset];
            }
        offset += blockDim.x * blockDim.y * blockDim.z;
    ...
    if (partIdx < N)
        ...
        for (unsigned int featureEpoch(0);
             featureEpoch < (maxFeatures + blockDim.y - 1) / blockDim.y;
             ++featureEpoch)
            {
            ...
            if (featureType == 0 && featureIdx < numTypeVerts[type_i])
                ...
                r0 = rotate(
                    quat_i,
                    vec3<Real>(vertices[edges[2 * (firstEdgeInType[type_i] + featureIdx)]]));
                r1 = rotate(
                    quat_i,
                    vec3<Real>(vertices[edges[2 * (firstEdgeInType[type_i] + featureIdx) + 1]]));
                }
            ...
            for (int neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++)
                ...
                if (evaluator.withinCutoff(rsq, r_cutsq))
                    ...
                    if (featureType == 0
                        && featureIdx < numTypeVerts[type_i]) // vertices of i and faces/edges of j
                        {
                        // faces of j
                        ...
                        // shape j wasn't a spherocylinder either, must be a sphere
                            ...
                            evaluator.vertexVertex(rij,
                             ...
                             && featureIdx
                                    < numTypeVerts[type_j]) // vertices of j and faces/edges of i
                        {
                        evaluator.swapij();
                        r0 = rotate(neighQuat,
                                    vec3<Real>(vertices[firstTypeVert[type_j] + featureIdx]));
                        ...
                        for (unsigned int edgeIdx(firstEdgeInType[type_j]);
                             edgeIdx < firstEdgeInType[type_j] + numTypeEdges[type_j];
                             ++edgeIdx)
                            {
                            const vec3<Real> r2(
                                rotate(neighQuat, vec3<Real>(vertices[edges[2 * edgeIdx]])));
                            const vec3<Real> r3(
                                rotate(neighQuat, vec3<Real>(vertices[edges[2 * edgeIdx + 1]])));
                            evaluator.edgeEdge(rij,
                                               r0,
                                               r1,
                                               rij + r2,
                                               rij + r3,
...
/*! \param d_force Device memory to write computed forces
  ...
  This is just a driver for gpu_compute_dem3d_forces_kernel, see the documentation for it for more
  information.
*/
template<typename Real, typename Real4, typename Evaluator>
hipError_t gpu_compute_dem3d_forces(Scalar4* d_force,
    ...
    hipFuncGetAttributes(
        &attr,
        reinterpret_cast<const void*>(gpu_compute_dem3d_forces_kernel<Real, Real4, Evaluator>));
    ...
    const size_t shmSize(
        2 * particlesPerBlock * sizeof(Real4) + 6 * particlesPerBlock * sizeof(Real)
        +                                     // forces, torques, virials per-particle
        ...
        numVerts * sizeof(Real4) + 2 * numEdges * sizeof(unsigned int)
        + // real vertex->point, edge->real index
        5 * numTypes * sizeof(unsigned int)
        + numVerts * sizeof(unsigned int)); // per-type counts and per-vertex connectivity

    // run the kernel
    hipLaunchKernelGGL((gpu_compute_dem3d_forces_kernel<Real, Real4, Evaluator>),
                       dim3(grid),
                       dim3(threads),
                       shmSize,
                       0,
