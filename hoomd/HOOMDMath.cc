...
void export_hoomd_math_functions(py::module& m)
    ...
    py::class_<uint2, std::shared_ptr<uint2> >(m,"uint2")
        .def(py::init<>())
        .def_property("x", [](const uint2& s) { return (unsigned int) s.x; }, [](uint2 & s, uint v) { s.x = v; })
        .def_property("y", [](const uint2& s) { return (unsigned int) s.y; }, [](uint2 & s, uint v) { s.y = v; })
        ;
    py::class_<uint3, std::shared_ptr<uint3> >(m,"uint3")
        .def(py::init<>())
        .def_property("x", [](const uint3& s) { return (unsigned int) s.x; }, [](uint3 & s, uint v) { s.x = v; })
        .def_property("y", [](const uint3& s) { return (unsigned int) s.y; }, [](uint3 & s, uint v) { s.y = v; })
        .def_property("z", [](const uint3& s) { return (unsigned int) s.z; }, [](uint3 & s, uint v) { s.z = v; })
        ;
    py::class_<uint4, std::shared_ptr<uint4> >(m,"uint4")
        .def(py::init<>())
        .def_property("x", [](const uint4& s) { return (unsigned int) s.x; }, [](uint4 & s, uint v) { s.x = v; })
        .def_property("y", [](const uint4& s) { return (unsigned int) s.y; }, [](uint4 & s, uint v) { s.y = v; })
        .def_property("z", [](const uint4& s) { return (unsigned int) s.z; }, [](uint4 & s, uint v) { s.z = v; })
        .def_property("w", [](const uint4& s) { return (unsigned int) s.w; }, [](uint4 & s, uint v) { s.w = v; })
        ;
