...
def test_wrap(s):
    ...
    if s.communicator.rank == 0:
        ...
        # set up particles outside of the box
        s.particles.N = 4
        s.particles.position[:] = [[-4, 0, 0], [2, 3, 1], [2, -8, 9], [100,200,-100]]
        # wrap
        s .wrap()
        # check the particles are where they should be
        numpy.testing.assert_allclose(s.particles.position,
    ...
    if s.communicator.rank == 0:
        ...
        # set up particles outside of the box
        s.particles.N = 4
        s.particles.position[:] = [[-4, 0, 0], [2, 5, 0], [7, 3, 4], [100, -50, -300]]
        # wrap
        s .wrap()
        # check the particles are where they should be
        numpy.testing.assert_allclose(s.particles.position,
    ...
    if s.communicator.rank == 0:
        ...
        # set up particles outside of the box
        s.particles.N = 4
        s.particles.position[:] = [[-8, 0, 0], [-4, -8, -7], [0, 2, 1], [50, 100, -300]]
        # wrap
        s .wrap()
        # check the particles are where they should be
        numpy.testing.assert_allclose(s.particles.position,
                                      [[-1, 0, 0], [3, -1, 1],
                                       [0, 2, 1], [0.9999995, 1.999999, 0]])
    if s.communicator.rank == 0:
        s.configuration.box = [8, 6, 4, 0, 0, 0] # orthorhombic box
        # set up particles outside of the box
        s.particles.N = 4
        s.particles.position[:] = [[-4, 3, 2], [0, -6, 10], [2, 0, -1], [-200, -200, -100]]
        # wrap
        s .wrap()
        # check the particles are where they should be
        numpy.testing.assert_allclose(s.particles.position,
    ...
    if s.communicator.rank == 0:
        ...
        # set up particles outside of the box
        s.particles.N = 4
        s.particles.position[:] = [[-4, 9, -9], [0, 4, 13], [2, 1, 1], [-200, -100, -100]]
        # wrap
        s .wrap()
        # check the particles are where they should be
        numpy.testing.assert_allclose(s.particles.position,
