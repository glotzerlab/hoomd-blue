...
class force_active_tests (unittest.TestCase):
    ...
    def test_create(self):
        ...
        force.active(seed=7, f_lst=activity, group=group.all())

    # tests options to force.active
    def test_options(self):
        np.random.seed(2,)
        activity = [ tuple(((np.random.rand(3) - 0.5) * 2.0)) for i in range(100)] # random forces
        force.active(seed=2, f_lst=activity, group=group.all(), rotation_diff=1.0, orientation_link=False)
        force.active(seed=2, f_lst=activity, group=group.all(), rotation_diff=0.0, orientation_link=True)
    ...
    def test_init_checks(self):
        ...
        act = force.active(seed=10, f_lst=activity, group=group.all());
        act.cpp_force = None;
