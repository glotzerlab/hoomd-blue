...
@pytest.mark.parametrize(
    "special_pair_cls, params, r_cut, force, energy", special_pair_test_parameters
)
...
def test_after_attaching(
    snapshot_factory, simulation_factory, special_pair_cls, params, r_cut, force, energy
):
...
def test_forces_and_energies(
    ...
    if sim.device.communicator.rank == 0:
        ...
        numpy.testing.assert_allclose(
            sim_forces[0], [force, 0.0, 0.0], rtol=1e-6, atol=1e-5
        )
        numpy.testing.assert_allclose(
            sim_forces[1], [-1 * force, 0.0, 0.0], rtol=1e-6, atol=1e-5
        )
...
@pytest.mark.parametrize(
    ...
    zip(
        (md.special_pair.SpecialPair, md.special_pair.LJ, md.special_pair.Coulomb),
...
def test_kernel_parameters(
    simulation_factory, snapshot_factory, special_pair_cls, params, r_cut, force, energy
):
    snapshot = snapshot_factory()
    sim = simulation_factory(snapshot)
