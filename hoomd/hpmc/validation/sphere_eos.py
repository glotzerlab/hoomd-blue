...
class sphereEOS_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mc_tune = hpmc.util.tune(self.mc,
                                 tunables=['d', 'a'],
                                 max_val=[4, 0.5],
                                 gamma=1,
                                 target=0.3)
        ...
        self.log = analyze.log(filename=None,
                               quantities=[
                                   'hpmc_overlap_count', 'volume', 'phi_p',
                                   'hpmc_d', 'hpmc_a', 'time'
                               ],
                               overwrite=True,
                               period=100)
        self.log.register_callback(
            'phi_p', lambda timestep: len(self.system.particles) * V / self.
        ...
        npt_tune = hpmc.util.tune_npt(boxmc,
                                      tunables=tunables,
                                      target=0.2,
                                      gamma=1)
        ...
        for i in range(10):
            ...
            if context.current.device.comm.rank == 0:
                print('d: {:3.2f} accept: {:3.2f} / v: {:3.2f} accept: {:3.2f}'
                      .format(d, translate_acceptance, v, volume_acceptance))
    ...
    def test_measure_phi_p(self):
        ...
        self.assertLessEqual(math.fabs(phi_p_avg - phi_p_ref[P]),
                             ci * (phi_p_ref[P] * rel_err_cs + phi_p_err))
