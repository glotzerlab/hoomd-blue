...
context.current.device.cpp_msg.notice(
    1, "parameter {} seed {} phi_c {:.3f} eta_p_r {:.3f}\n".format(
        p, seed, phi_c, eta_p_r))
...
class implicit_test(unittest.TestCase):
    ...
    def test_sphere_ntrial3(
            self):  # we could coment out this one if the CI takes too long
    ...
    def measure_etap_sphere(self, use_clusters, ntrial):
        ...
        if not use_clusters:
            self.mc_tune = hpmc.util.tune(self.mc,
                                          tunables=['d'],
                                          max_val=[d_sphere],
                                          gamma=1,
                                          target=0.2)
        ...
        free_volume = hpmc.compute.free_volume(mc=self.mc,
                                               seed=seed,
                                               nsample=10000,
                                               test_type='B')
        log = analyze.log(filename=None,
                          quantities=[
                              'hpmc_overlap_count', 'volume',
                              'hpmc_free_volume', 'hpmc_fugacity_B'
                          ],
                          overwrite=True,
                          period=100)
        ...
        def log_callback(timestep):
            v = math.pi / 6.0 * log.query('hpmc_free_volume') / log.query(
                'volume') * log.query('hpmc_fugacity_B')
        ...
        if context.current.device.comm.rank == 0:
            ...
            print('tgt: {:.6f} +- {:.6f}'.format(eta_p_ref[(phi_c, eta_p_r)][0],
                                                 eta_p_ref[(phi_c,
                                                            eta_p_r)][1]))
    ...
    def measure_etap_sphere_union(self, use_clusters, ntrial):
        ...
        self.mc.shape_param.set('A',
                                centers=[(0, 0, 0)] * 2,
                                diameters=[d_sphere, 0.5 * d_sphere],
                                capacity=1)
        self.mc.shape_param.set('B',
                                centers=[(0, 0, 0)],
                                diameters=[d_sphere * q],
                                capacity=1)
