...
UpdaterMuVTImplicit<Shape, Integrator>::UpdaterMuVTImplicit(std::shared_ptr<SystemDefinition> sysdef,
    ...
    unsigned int seed,
    unsigned int npartition)
    : UpdaterMuVT<Shape>(sysdef, mc_implicit,seed,npartition), m_mc_implicit(mc_implicit), m_n_trial(1)
    {
    }
...
bool UpdaterMuVTImplicit<Shape,Integrator>::trySwitchType(unsigned int timestep, unsigned int tag, unsigned int new_type,
    ...
    for (unsigned int type_d = 0; type_d < this->m_pdata->getNTypes(); ++type_d)
        ...
        unsigned int n_overlap = countDepletantOverlapsInNewPosition(timestep, n_dep, delta, pos, orientation, new_type, tmp_free, type_d);
        ...
        if (this->m_gibbs)
            ...
            if (nonzero)
                ...
                if (moveDepletantsIntoNewPosition(timestep, n_overlap, delta, pos, orientation, new_type, m_n_trial, lnb, type_d))
                    {
                    lnboltzmann -= lnb;
                    }
                else
            ...
            if (moveDepletantsInUpdatedRegion(timestep, n_insert, delta_old, tag, new_type, m_n_trial, lnb, type_d))
                {
                lnboltzmann += lnb;
                }
            else
...
unsigned int UpdaterMuVTImplicit<Shape,Integrator>::getNumDepletants(unsigned int timestep,  Scalar V, bool local,
    ...
    Scalar lambda = this->m_mc_implicit->getDepletantFugacity(type_d)*V;
...
void export_UpdaterMuVTImplicit(pybind11::module& m, const std::string& name)
    {
    pybind11::class_< hpmc::UpdaterMuVTImplicit<Shape, Integrator>, std::shared_ptr< hpmc::UpdaterMuVTImplicit<Shape, Integrator> > >(m, name.c_str(),
          pybind11::base<hpmc::UpdaterMuVT<Shape> >())
          .def(pybind11::init< std::shared_ptr<SystemDefinition>,
            std::shared_ptr< Integrator >, unsigned int, unsigned int>())
          .def("setNTrial",&hpmc::UpdaterMuVTImplicit<Shape, Integrator>::setNTrial)
          ;
    }
