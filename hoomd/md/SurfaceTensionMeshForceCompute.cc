...
Scalar SurfaceTensionMeshForceCompute::energyDiff(unsigned int idx_a,
    ...
    Scalar3 dcd;
    dcd.x = h_pos.data[idx_d].x - h_pos.data[idx_c].x;
    dcd.y = h_pos.data[idx_d].y - h_pos.data[idx_c].y;
    dcd.z = h_pos.data[idx_d].z - h_pos.data[idx_c].z;
    ...
    Scalar rabrac = dab.x * dac.x + dab.y * dac.y + dab.z * dac.z;
    Scalar rbarbd = dab.x * dbd.x + dab.y * dbd.y + dab.z * dbd.z;

    Scalar rcdrca = dcd.x * dac.x + dcd.y * dac.y + dcd.z * dac.z;
    Scalar rdcrdb = dcd.x * dbd.x + dcd.y * dbd.y + dcd.z * dbd.z;

    Scalar area_old = sqrt(rsqab * rsqac - rabrac * rabrac) + sqrt(rsqab * rsqbd - rbarbd * rbarbd);
    Scalar area_new = sqrt(rsqcd * rsqac - rcdrca * rcdrca) + sqrt(rsqcd * rsqbd - rdcrdb * rdcrdb);

    return m_sigma[0] * (area_new - area_old);
    }
