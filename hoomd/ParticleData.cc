...
/*! Loads particle data from the snapshot into the internal arrays, version for hyperspherical coordinates
 * \param snapshot The particle data snapshot
 * \param hypersphere The hypersphere of the system, which determines the curvature of the space
 * \param exec_conf The execution configuration
 */
template <class Real>
ParticleData::ParticleData(const SnapshotParticleData<Real>& snapshot,
    ...
    : m_exec_conf(exec_conf),
      ...
      m_o_image(make_int3(0,0,0)),
      m_arrays_allocated(false),
      m_coordinate(hyperspherical)
    {
    m_exec_conf->msg->notice(5) << "Constructing ParticleData" << endl;

    // initialize box dimensions on all procesors
    setHypersphere(hypersphere);
...
std::map<unsigned int, unsigned int> ParticleData::takeSnapshot(SnapshotParticleData<Real> &snapshot)
    ...
    snapshot.use_hyperspherical_coord = getCoordinateType() == hyperspherical;
...
void export_Hypersphere(py::module& m)
    {
...
void export_ParticleData(py::module& m)
    ...
    .def("getHypersphere", &ParticleData::getHypersphere, py::return_value_policy::reference_internal)
    .def("setHypersphere", &ParticleData::setHypersphere)
    ...
    .value("hyperspherical", ParticleData::coordinate_Enum::hyperspherical)
    .export_values()
    ;
    }
...
SnapshotParticleData<Real>::SnapshotParticleData(unsigned int N)
       : size(N), is_accel_set(false), use_hyperspherical_coord(false)
    {
    resize(N);
    }
...
*/
template <class Real>
py::object SnapshotParticleData<Real>::getQuatLNP(pybind11::object self)
    {
    auto self_cpp = self.cast<SnapshotParticleData<Real> *>();
    // mark as dirty when accessing internal data
    self_cpp->is_accel_set = false;
    ...
    std::vector<size_t> dims(2);
    dims[0] = self_cpp->quat_l.size();
    dims[1] = 4;
    return pybind11::array(dims,(Real*)&self_cpp->quat_l[0], self);
    }
...
*/
template <class Real>
py::object SnapshotParticleData<Real>::getQuatRNP(pybind11::object self)
    {
    auto self_cpp = self.cast<SnapshotParticleData<Real> *>();
    // mark as dirty when accessing internal data
    self_cpp->is_accel_set = false;
    ...
    std::vector<size_t> dims(2);
    dims[0] = self_cpp->quat_r.size();
    dims[1] = 4;
    return pybind11::array(dims,(Real*)&self_cpp->quat_r[0], self);
    }
...
void SnapshotParticleData<Real>::bcast(unsigned int root, MPI_Comm mpi_comm)
    ...
    ::bcast(use_hyperspherical_coord, root, mpi_comm);
    }
#endif
...
void export_SnapshotParticleData(py::module& m)
    ...
    .def_property_readonly("quat_l", &SnapshotParticleData<float>::getQuatLNP)
    .def_property_readonly("quat_r", &SnapshotParticleData<float>::getQuatRNP)
    ...
    .def_readwrite("use_hyperspherical_coord", &SnapshotParticleData<float>::use_hyperspherical_coord)
    ;
    ...
    .def_property_readonly("quat_l", &SnapshotParticleData<double>::getQuatLNP)
    .def_property_readonly("quat_r", &SnapshotParticleData<double>::getQuatRNP)
    ...
    .def_readwrite("use_hyperspherical_coord", &SnapshotParticleData<double>::use_hyperspherical_coord)
    ;
   }
