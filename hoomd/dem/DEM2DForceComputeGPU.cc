...
DEM2DForceComputeGPU<Real, Real2, Real4, Potential>::DEM2DForceComputeGPU(
    ...
    if (!this->m_exec_conf->isCUDAEnabled())
        ...
        this->m_exec_conf->msg->error()
            << "Creating a DEM2DForceComputeGPU with no GPU in the execution configuration" << endl;
    ...
    m_tuner.reset(
        new Autotuner(warp_size, max_threads, warp_size, 5, 100000, "dem_2d", this->m_exec_conf));
    }
...
void DEM2DForceComputeGPU<Real, Real2, Real4, Potential>::computeForces(uint64_t timestep)
    ...
    if (third_law)
        {
        this->m_exec_conf->msg->error()
            << "DEM2DForceComputeGPU cannot handle a half neighborlist" << endl;
    ...
    ArrayHandle<Scalar4> d_pos(this->m_pdata->getPositions(),
                               access_location::device,
                               access_mode::read);
    ArrayHandle<Scalar4> d_quat(this->m_pdata->getOrientationArray(),
                                access_location::device,
                                access_mode::read);
    ArrayHandle<Scalar> d_diam(this->m_pdata->getDiameters(),
                               access_location::device,
                               access_mode::read);
    ArrayHandle<Scalar4> d_velocity(this->m_pdata->getVelocities(),
                                    access_location::device,
                                    access_mode::read);
    ArrayHandle<Real2> d_vertices(m_vertices, access_location::device, access_mode::read);
    ArrayHandle<unsigned int> d_num_shape_vertices(m_num_shape_vertices,
                                                   access_location::device,
                                                   access_mode::read);
    ...
    m_tuner->begin();
    kernel::gpu_compute_dem2d_forces<Real, Real2, Real4, Evaluator>(
        d_force.data,
        d_torque.data,
        d_virial.data,
...
void DEM2DForceComputeGPU<Real, Real2, Real4, Potential>::createGeometry()
    ...
    ArrayHandle<Real2> h_vertices(m_vertices, access_location::host, access_mode::overwrite);
    ArrayHandle<unsigned int> h_num_shape_vertices(m_num_shape_vertices,
...
size_t DEM2DForceComputeGPU<Real, Real2, Real4, Potential>::numVertices() const
    ...
    for (typename std::vector<std::vector<vec2<Real>>>::const_iterator shapeIter(
             this->m_shapes.begin());
