...
def logger():
    ...
    logger[("dummy", "loggable", "small_float")] = (Identity(0.0000001), "scalar")
...
def test_header_generation(device, logger):
    ...
    expected_headers = [
        "dummy.loggable.int",
        "dummy.loggable.float",
        "dummy.loggable.string",
...
def test_max_precision(device, logger):
    output = StringIO("")
    table_writer = hoomd.write.Table(1, logger, output, pretty=False, max_precision=5)
    table_writer._comm = device.communicator
    for i in range(10):
        table_writer.write()
    ...
    output = StringIO("")
    table_writer = hoomd.write.Table(1, logger, output, pretty=False, max_precision=15)
    table_writer._comm = device.communicator
    for i in range(10):
        table_writer.write()
...
combinations = [
    set(combo)
    for i in range(1, len(test_categories) + 1)
