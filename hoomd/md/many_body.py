...
class Tersoff(_ThreeBody):
    ...
    Warning:
        ...
        Since reverse force communication is required for the calculation of multi-body potentials, attempting to use this
...
class RevCross(_ThreeBody):
    ...
    .. attention::
        ...
        In our example we then set the interactions for types A and B with ``rev_c.params[[('A','B'),('A','B')]] = dict(sigma=0.0,n=0,epsilon=0,lambda3=0)``
        and the only non-zero energy only between the different types ``rev_c.params[('A','B')] = dict(sigma=1,n=100,epsilon=100,lambda3=1)``.
        Notice that the number of the minority species corresponds to the maximum number of bonds.
