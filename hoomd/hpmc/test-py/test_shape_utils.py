...
class mass_properties_convex_polyhedron_test(unittest.TestCase):
    ...
    def test_convex_polyhedron(self):
        ...
        for _ in range(100):
            ...
            self.assertAlmostEqual(inertia[0], cpp_inertia[0], 4)
            # xx
            self.assertAlmostEqual(inertia[3], cpp_inertia[1], 4)
            # yy
            self.assertAlmostEqual(inertia[5], cpp_inertia[2], 4)
            # zz
            self.assertAlmostEqual(inertia[1], cpp_inertia[3], 4)
            # xy
            self.assertAlmostEqual(inertia[4], cpp_inertia[4], 4)
            # yz
            self.assertAlmostEqual(inertia[2], cpp_inertia[5], 4)
            # xz
        print("c++ ran 100 convex hulls in {} ({} per call)".format(
            cpp_time, cpp_time / 100))
        print("py ran 100 convex hulls in {} ({} per call)".format(
            py_time, py_time / 100))
