...
_cosinesq_args = {'k': [3.0, 10.0, 5.0],
                  't0': [np.pi / 2, np.pi / 4, np.pi / 6]}


def get_angle_and_args():
    harmonic_arg_list = [dict(zip(_harmonic_args, val)) for val in zip(*_harmonic_args.values())]
    cosinesq_arg_list = [dict(zip(_cosinesq_args, val)) for val in zip(*_cosinesq_args.values())]
    angle_and_args = []
...
def get_angle_args_forces_and_energies():
    ...
    for i in range(3):
        angle_args_forces_and_energies.append((hoomd.md.angle.Harmonic,
                                               harmonic_arg_list[i],
                                               harmonic_forces[i],
                                               harmonic_energies[i]))
    for i in range(3):
        angle_args_forces_and_energies.append((hoomd.md.angle.Cosinesq,
                                               cosinesq_arg_list[i],
                                               cosinesq_forces[i],
                                               cosinesq_energies[i]))
...
@pytest.mark.parametrize("angle_and_args", get_angle_and_args())
def test_after_attaching(triplet_snapshot_factory, simulation_factory, angle_and_args):
...
@pytest.mark.parametrize("angle_args_force_and_energy", get_angle_args_forces_and_energies())
def test_after_attaching(triplet_snapshot_factory, simulation_factory, angle_args_force_and_energy):
    theta_deg = 60
    theta_rad = theta_deg * (np.pi / 180)
