...
namespace hpmc
    {
    PairPotentialUnion::PairPotentialUnion(std::shared_ptr<SystemDefinition> sysdef, std::shared_ptr<PairPotential> constituent_potential)
    : PairPotential(sysdef),
      m_constituent_potential(constituent_potential),
        m_leaf_capacity(4)
    {
...
void PairPotentialUnion::setBody(std::string body_type, pybind11::object v)
    ...
    m_type[body_type_id].resize(N);
        ...
        for (unsigned int i = 0; i < N; i++)
            ...
            m_position[body_type_id][i] = vec3<ShortReal>(r_python[0].cast<ShortReal>(), r_python[1].cast<ShortReal>(), r_python[2].cast<ShortReal>());

            if (orientations.is_none())
                {
                m_orientation[body_type_id][i] = quat<ShortReal>(ShortReal(1.0), vec3<ShortReal>(0, 0, 0));
                }
            else
                {
            ...
            if (pybind11::len(q_python) != 4)
                ...
                m_orientation[body_type_id][i] = quat<ShortReal>(q_python[0].cast<ShortReal>(),
                vec3<ShortReal>(q_python[1].cast<ShortReal>(),
                                q_python[2].cast<ShortReal>(),
                                q_python[3].cast<ShortReal>()));
                }

        m_type[body_type_id][i] = m_sysdef->getParticleData()->getTypeByName(types[i].cast<std::string>());
...
pybind11::object PairPotentialUnion::getBody(std::string body_type)
    ...
    pybind11::list charges;
        ...
        for (unsigned int i = 0; i < N; i++)
            ...
            orientations.append(
                pybind11::make_tuple(m_orientation[body_type_id][i].s,
...
LongReal PairPotentialUnion::compute_leaf_leaf_energy(vec3<ShortReal> dr,
    ...
    for (unsigned int i = 0; i < na; i++)
        ...
        quat<ShortReal> orientation_i = conj(quat<ShortReal>(orientation_b)) * quat<ShortReal>(orientation_a)
                                    * m_orientation[type_a][ileaf];
        vec3<ShortReal> pos_i(rotate(conj(quat<ShortReal>(orientation_b)) * quat<ShortReal>(orientation_a),
