...
struct gsd_schema_hpmc : public gsd_schema_hpmc_base
    ...
    int write(gsd_handle& handle, const std::string& name, unsigned int Ntypes, const float* const d, const float* const a)
        {
        int retval = 0;
        std::string path = name + "/d";
        if(d)
            retval |= gsd_write_chunk(&handle, path.c_str(), GSD_TYPE_FLOAT, Ntypes, 1, 0, (void *)d);
        path = name + "/a";
        if(a)
            retval |= gsd_write_chunk(&handle, path.c_str(), GSD_TYPE_FLOAT, Ntypes, 1, 0, (void *)a);
        return retval;
        }
    bool read(std::shared_ptr<GSDReader>, uint64_t, const std::string&, unsigned int, const float* const d, const float* const a)
        {
