...
*/
template< class Shape >
cudaError_t gpu_hpmc_insert_depletants_queue(const hpmc_implicit_args_t& args, const typename Shape::param_type *params)
    {
    assert(args.d_postype);
    assert(args.d_orientation);
...
//! Kernel driver for gpu_hpmc_implict_accept_reject_kernel()
/*! \param args Bundled arguments
    \param d_params Per-type shape parameters
    \returns Error codes generated by any CUDA calls, or cudaSuccess when there is no error
