...
struct PYBIND11_EXPORT AABBNode
    ...
    unsigned int
        skip; //!< Number of array indices to skip to get to the next node in an in order traversal
...
//! AABB Tree
/*! An AABBTree stores a binary tree of AABBs. A leaf node stores up to NODE_CAPACITY particles by
   index. The bounding box of a leaf node surrounds all the bounding boxes of its contained
   particles. Internal nodes have AABBs that enclose all of their children. The tree supports the
   following operations:

    - Query  : Search through the tree and build a list of all particles that intersect with the
   query AABB. Runs in O(log N) time
    - Update : Update the AABB for a selected particle. Updating works well only for small movements
   as the tree topology is left unchanged. Runs in O(log N) time. AABBs are not saved for all
   particles, so an update will only increase the volume of nodes. The tree should be rebuilt
   periodically instead of continually updated.
    - buildTree : build an efficiently arranged tree given a complete set of AABBs, one for each
   particle.

    **Implementation details**

    AABBTree stores all nodes in a flat array managed by std::vector. To easily locate particle leaf
   nodes for update, a reverse mapping is stored to locate the leaf node containing a particle.
   m_root tracks the index of the root node as the tree is built. The nodes store the indices of
   their left and right children along with their AABB. Nodes are allocated as needed with
   allocate(). With multiple particles per leaf node, the total number of internal nodes needed is
   not known (but can be estimated) until build time.
...
class PYBIND11_EXPORT AABBTree
    ...
    inline unsigned int buildNode(AABB* aabbs,
...
/*! \param hits Output vector of positive hits.
    ...
    The *hits* vector is not cleared, elements are only added with push_back. query() traverses the
   tree and finds all of the leaf nodes that intersect *aabb*. The index of each intersecting leaf
   node is added to the hits vector.
*/
...
/*! \param idx Particle index to update
    ...
    Update the node for particle *idx* and its parent nodes to reflect a new position and/or
   extents. update() does not change the tree topology, so it is best for slight changes.
*/
...
/*! \param aabbs List of AABBs for each particle (must be 32-byte aligned)
    ...
    Builds a balanced tree from a given list of AABBs for each particle. Data in \a aabbs will be
...
/*! \param aabbs List of AABBs
    ...
    buildNode is the main driver of the smart AABB tree build algorithm. Each call produces a node,
   given a set of AABBs. If there are fewer AABBs than fit in a leaf, a leaf is generated. If there
   are too many, the total AABB is computed and split on the largest length axis. The total tree is
   built by recursive splitting.

    The aabbs and idx lists are passed in by reference. Each node is given a subrange of the list to
   own (start to start + len). When building the node, it partitions its subrange into two sides
   (like quick sort).
*/
...
/*! \param idx Index of the node to update
    ...
    updateSkip() updates the skip field of every node in the tree. The skip field is used in the
   stackless implementation of query. Each node's skip field lists the number of nodes that are
   children to this node. Because of the order in which nodes are built in buildNode(), this number
   is the number of elements to skip in a search if a box-box test does not overlap.
*/
...
inline unsigned int AABBTree::allocateNode()
    ...
    if (m_num_nodes >= m_node_capacity)
        ...
        // allocate new memory
        int retval
            = posix_memalign((void**)&m_new_nodes, 32, m_new_node_capacity * sizeof(AABBNode));
        if (retval != 0)
            {
            throw std::runtime_error("Error allocating AABBTree memory");
            }
