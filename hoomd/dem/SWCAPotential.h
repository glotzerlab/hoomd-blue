...
/*! Pluggable potential for a WCA interaction, shifted by the diameter of each particle.
  ...
  \f}
  where \f$ \Delta = (d_i + d_j)/2 - 1 \f$ and \f$ d_i \f$ is the assigned diameter of particle \f$
  i \f$.
*/
template<typename Real, typename Real4, typename FrictionModel> class SWCAPotential
    {
    public:
    ...
    SWCAPotential(Real radius, const FrictionModel& frictionParams)
        : m_radius(radius), m_sigma6(radius * radius * radius * radius * radius * radius * 64),
          m_rcutsq(radius * radius * 4 * pow(2.0, 1. / 3)), m_frictionParams(frictionParams)
        {
        }
    ...
    DEVICE void setDiameter(Real di, Real dj)
        {
        m_delta = 0.5 * (di + dj) - 1;
        }
