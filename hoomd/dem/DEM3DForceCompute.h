...
/*! The total pair force is summed for each particle when compute() is called. Forces are only
  ...
  Usage: Construct a DEM3DForceCompute, providing it an already constructed ParticleData and
  NeighborList. Then set parameters for all possible pairs of types by calling setParams.
  ...
  - The first face of the type with type index i is stored at index i within the face->next face
  array
...
class DEM3DForceCompute : public ForceCompute
    ...
    virtual void
    setParams(unsigned int type, const pybind11::list& pyVertices, const pybind11::list& pyFaces);
    ...
    std::vector<std::string> getTypeShapeMapping(const std::vector<std::vector<vec3<Real>>>& verts,
    ...
    GPUArray<unsigned int>
        m_vertexConnectivity;       //!< real vertex index->number of times it appears in an edge
