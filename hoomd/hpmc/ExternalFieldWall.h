...
struct SphereWall
    {
    SphereWall() : rsq(0), inside(false), origin(0,0,0), verts(new detail::poly3d_full(1, 0, 0, false)) {}
    SphereWall(Scalar r, vec3<Scalar> orig, bool ins = true) : rsq(r*r), inside(ins), origin(orig), verts(new detail::poly3d_full(1, 0, 0, false))
    {
    ...
    SphereWall(const SphereWall& src) : rsq(src.rsq), inside(src.inside), origin(src.origin), verts(new detail::poly3d_full(*src.verts)) {}
    ...
    std::shared_ptr<detail::poly3d_full >    verts;
    };

struct CylinderWall
    {
    CylinderWall() : rsq(0), inside(false), origin(0,0,0), orientation(origin), verts(new detail::poly3d_full) {}
    CylinderWall(Scalar r, vec3<Scalar> orig, vec3<Scalar> orient, bool ins = true) : rsq(0), inside(false), origin(0,0,0), orientation(origin), verts(new detail::poly3d_full(2, 0, 0, false))
        {
    ...
    CylinderWall(const CylinderWall& src) : rsq(src.rsq), inside(src.inside), origin(src.origin), orientation(src.orientation), verts(new detail::poly3d_full(*src.verts)) {}
    // scale all distances associated with the sphere wall by some factor alpha
    void scale(const OverlapReal& alpha)
        {
        rsq *= alpha*alpha;
