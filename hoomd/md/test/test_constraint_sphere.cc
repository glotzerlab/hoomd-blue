...
void constraint_sphere_tests(cs_creator_t cs_creator,
    ...
    std::shared_ptr<SystemDefinition> sysdef(
        new SystemDefinition(6, BoxDim(1000000.0), 1, 0, 0, 0, 0, exec_conf));
    ...
    // run the particles in a BD simulation with a constraint force applied and verify that the
...
UP_TEST(BDUpdater_tests)
    ...
    constraint_sphere_tests(cs_creator,
