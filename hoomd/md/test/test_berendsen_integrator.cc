...
void berend_updater_lj_tests(std::shared_ptr<ExecutionConfiguration> exec_conf)
    {
    // check that the berendsen thermostat applied to a system of 1000 LJ particles produces the
    // correct average temperature Build a 1000 particle system with particles scattered on the x,
    // y, and z axes.
...
UP_TEST(TwoStepBerendsen_LJ_tests)
    {
    berend_updater_lj_tests<TwoStepBerendsen>(std::shared_ptr<ExecutionConfiguration>(
...
UP_TEST(TwoStepBerendsenGPU_LJ_tests)
    {
    berend_updater_lj_tests<TwoStepBerendsenGPU>(std::shared_ptr<ExecutionConfiguration>(
