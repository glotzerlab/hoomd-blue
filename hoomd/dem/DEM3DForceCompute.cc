...
DEM3DForceCompute<Real, Real4, Potential>::DEM3DForceCompute(
    ...
    : ForceCompute(sysdef), m_nlist(nlist), m_r_cut(r_cut), m_evaluator(potential),
      m_nextFace(0, this->m_exec_conf), m_firstFaceVert(0, this->m_exec_conf),
      m_nextFaceVert(0, this->m_exec_conf), m_realVertIndex(0, this->m_exec_conf),
      m_firstTypeVert(0, this->m_exec_conf), m_numTypeVerts(0, this->m_exec_conf),
      m_firstTypeEdge(0, this->m_exec_conf), m_numTypeEdges(0, this->m_exec_conf),
      m_numTypeFaces(0, this->m_exec_conf), m_vertexConnectivity(0, this->m_exec_conf),
      m_edges(0, this->m_exec_conf), m_faceRcutSq(0, this->m_exec_conf),
      m_edgeRcutSq(0, this->m_exec_conf), m_verts(0, this->m_exec_conf), m_shapes(), m_facesVec()
    {
...
template<typename Real, typename Real4, typename Potential>
void DEM3DForceCompute<Real, Real4, Potential>::connectDEMGSDShapeSpec(
    std::shared_ptr<GSDDumpWriter> writer)
    {
    typedef hoomd::detail::SharedSignalSlot<int(gsd_handle&)> SlotType;
    auto func = std::bind(&DEM3DForceCompute<Real, Real4, Potential>::slotWriteDEMGSDShapeSpec,
                          this,
                          std::placeholders::_1);
...
template<typename Real, typename Real4, typename Potential>
std::string
DEM3DForceCompute<Real, Real4, Potential>::getTypeShape(const std::vector<vec3<Real>>& verts,
    ...
    if (nverts == 1)
        ...
        {
        shapedef << "{\"type\": \"ConvexPolyhedron\", "
...
template<typename Real, typename Real4, typename Potential>
std::string DEM3DForceCompute<Real, Real4, Potential>::encodeVertices(
    const std::vector<vec3<Real>>& verts) const
    {
...
template<typename Real, typename Real4, typename Potential>
std::vector<std::string> DEM3DForceCompute<Real, Real4, Potential>::getTypeShapeMapping(
    const std::vector<std::vector<vec3<Real>>>& verts,
    const Real& radius) const
    {
...
*/
template<typename Real, typename Real4, typename Potential>
void DEM3DForceCompute<Real, Real4, Potential>::setParams(unsigned int type,
...
void DEM3DForceCompute<Real, Real4, Potential>::createGeometry()
    ...
    ArrayHandle<Real4> h_verts(m_verts, access_location::host, access_mode::overwrite);
    ArrayHandle<Real> h_faceRcutSq(m_faceRcutSq, access_location::host, access_mode::overwrite);
    ArrayHandle<Real> h_edgeRcutSq(m_edgeRcutSq, access_location::host, access_mode::overwrite);
    ArrayHandle<unsigned int> h_nextFaceVert(m_nextFaceVert,
                                             access_location::host,
                                             access_mode::overwrite);
    ArrayHandle<unsigned int> h_realVertIndex(m_realVertIndex,
                                              access_location::host,
                                              access_mode::overwrite);
    ArrayHandle<unsigned int> h_nextFace(m_nextFace, access_location::host, access_mode::overwrite);
    ArrayHandle<unsigned int> h_firstFaceVert(m_firstFaceVert,
                                              access_location::host,
                                              access_mode::overwrite);
    ArrayHandle<unsigned int> h_firstTypeVert(m_firstTypeVert,
                                              access_location::host,
                                              access_mode::overwrite);
    ArrayHandle<unsigned int> h_numTypeVerts(m_numTypeVerts,
                                             access_location::host,
                                             access_mode::overwrite);
    ArrayHandle<unsigned int> h_firstTypeEdge(m_firstTypeEdge,
                                              access_location::host,
                                              access_mode::overwrite);
    ArrayHandle<unsigned int> h_numTypeEdges(m_numTypeEdges,
                                             access_location::host,
                                             access_mode::overwrite);
    ArrayHandle<unsigned int> h_numTypeFaces(m_numTypeFaces,
                                             access_location::host,
                                             access_mode::overwrite);
    ArrayHandle<unsigned int> h_vertexConnectivity(m_vertexConnectivity,
                                                   access_location::host,
                                                   access_mode::overwrite);
    ArrayHandle<unsigned int> h_edges(m_edges, access_location::host, access_mode::overwrite);
    ...
    // build m_nextFace
    for (size_t shapeIdx(0), faceCount(0); shapeIdx < m_facesVec.size(); ++shapeIdx)
        {
    ...
    for (size_t shapeIdx(0), vertCount(0); shapeIdx < m_facesVec.size(); ++shapeIdx)
        {
        for (size_t faceIdx(shapeIdx), vecIdx(0); vecIdx < m_facesVec[shapeIdx].size();
    ...
    for (size_t shapeIdx(0), vertCount(0), vertTypeOffset(0); shapeIdx < m_facesVec.size();
         ++shapeIdx)
        {
        for (size_t faceIdx(0); faceIdx < m_facesVec[shapeIdx].size(); ++faceIdx)
            {
            for (size_t vertIdx(0); vertIdx < m_facesVec[shapeIdx][faceIdx].size();
                 ++vertIdx, ++vertCount)
                h_realVertIndex.data[vertCount]
                    = (unsigned int)(vertTypeOffset + m_facesVec[shapeIdx][faceIdx][vertIdx]);
            }
    ...
    for (size_t shapeIdx(0), edgeCount(0), vertTypeOffset(0); shapeIdx < m_facesVec.size();
         ++shapeIdx)
        {
        ...
        for (std::set<edge>::const_iterator edgeIter(edges.begin()); edgeIter != edges.end();
             ++edgeIter, ++edgeCount)
            {
...
size_t DEM3DForceCompute<Real, Real4, Potential>::numVertices() const
    ...
    for (typename std::vector<std::vector<vec3<Real>>>::const_iterator shapeIter(
             this->m_shapes.begin());
         shapeIter != this->m_shapes.end();
         ++shapeIter)
...
size_t DEM3DForceCompute<Real, Real4, Potential>::numFaces() const
    ...
    for (typename std::vector<std::vector<std::vector<unsigned int>>>::const_iterator shapeIter(
             this->m_facesVec.begin());
         shapeIter != this->m_facesVec.end();
         ++shapeIter)
        result += shapeIter->size() ? shapeIter->size() : 1;
...
size_t DEM3DForceCompute<Real, Real4, Potential>::numEdges() const
    ...
    for (ShapeIter shapeIter(this->m_facesVec.begin()); shapeIter != this->m_facesVec.end();
         ++shapeIter)
        {
        std::set<std::pair<unsigned int, unsigned int>> edges;
        for (FaceIter faceIter(shapeIter->begin()); faceIter != shapeIter->end(); ++faceIter)
            {
            for (VertIter vertIter(faceIter->begin()); (vertIter + 1) != faceIter->end();
                 ++vertIter)
                {
            ...
            unsigned int smaller(faceIter->back() < faceIter->front() ? faceIter->back()
                                                                      : faceIter->front());
            unsigned int larger(faceIter->back() < faceIter->front() ? faceIter->front()
                                                                     : faceIter->back());
            std::pair<unsigned int, unsigned int> edge(smaller, larger);
            if (smaller != larger)
                edges.insert(edge);
...
size_t DEM3DForceCompute<Real, Real4, Potential>::numDegenerateVerts() const
    ...
    for (typename std::vector<std::vector<std::vector<unsigned int>>>::const_iterator shapeIter(
         ...
         ++shapeIter)
        {
        for (typename std::vector<std::vector<unsigned int>>::const_iterator faceIter(
                 shapeIter->begin());
             faceIter != shapeIter->end();
             ++faceIter)
...
void DEM3DForceCompute<Real, Real4, Potential>::computeForces(uint64_t timestep)
    ...
    // GPU array handles
    ArrayHandle<Real4> h_verts(m_verts, access_location::host, access_mode::read);
    ArrayHandle<Real> h_faceRcutSq(m_faceRcutSq, access_location::host, access_mode::read);
    ArrayHandle<Real> h_edgeRcutSq(m_edgeRcutSq, access_location::host, access_mode::read);
    ArrayHandle<unsigned int> h_nextFaceVert(m_nextFaceVert,
                                             access_location::host,
                                             access_mode::read);
    ArrayHandle<unsigned int> h_realVertIndex(m_realVertIndex,
                                              access_location::host,
                                              access_mode::read);
    ArrayHandle<unsigned int> h_nextFace(m_nextFace, access_location::host, access_mode::read);
    ArrayHandle<unsigned int> h_firstFaceVert(m_firstFaceVert,
                                              access_location::host,
                                              access_mode::read);
    ArrayHandle<unsigned int> h_firstTypeVert(m_firstTypeVert,
    ...
    ArrayHandle<unsigned int> h_vertexConnectivity(m_vertexConnectivity,
                                                   access_location::host,
                                                   access_mode::read);
    ArrayHandle<unsigned int> h_edges(m_edges, access_location::host, access_mode::read);
            ...
            if (m_evaluator.withinCutoff(rsq, r_cut_sq))
                ...
                for (size_t vertIndex(0); vertIndex < h_numTypeVerts.data[typei]; ++vertIndex)
                    ...
                    const vec3<Real> vertex0(
                        rotate(quati,
                               vec3<Real>(h_verts.data[h_firstTypeVert.data[typei] + vertIndex])));
                    ...
                    // no faces; is it a spherocylinder?
                        ...
                        for (size_t edgej(0); edgej < h_numTypeEdges.data[typej]; ++edgej)
                            ...
                            vec3<Real> p10(
                                h_verts
                                    .data[h_edges.data[2 * (edgej + h_firstTypeEdge.data[typej])]]);
                            vec3<Real> p11(
                                h_verts.data[h_edges.data[2 * (edgej + h_firstTypeEdge.data[typej])
                                                          + 1]]);
                ...
                for (size_t vertIndex(0); vertIndex < h_numTypeVerts.data[typej]; ++vertIndex)
                    ...
                    const vec3<Real> vertex0(
                        rotate(quatj,
                               vec3<Real>(h_verts.data[h_firstTypeVert.data[typej] + vertIndex])));
                    ...
                    // no faces; is it a spherocylinder?
                        ...
                        for (size_t edgei(0); edgei < h_numTypeEdges.data[typei]; ++edgei)
                            ...
                            vec3<Real> p10(
                                h_verts
                                    .data[h_edges.data[2 * (edgei + h_firstTypeEdge.data[typei])]]);
                            vec3<Real> p11(
                                h_verts.data[h_edges.data[2 * (edgei + h_firstTypeEdge.data[typei])
                                                          + 1]]);
                ...
                for (size_t edgei(0); edgei < h_numTypeEdges.data[typei]; ++edgei)
                    ...
                    vec3<Real> p00(
                        h_verts.data[h_edges.data[2 * (edgei + h_firstTypeEdge.data[typei])]]);
                    vec3<Real> p01(
                        h_verts.data[h_edges.data[2 * (edgei + h_firstTypeEdge.data[typei]) + 1]]);
                    ...
                    for (size_t edgej(0); edgej < h_numTypeEdges.data[typej]; ++edgej)
                        ...
                        vec3<Real> p11(
                            h_verts
                                .data[h_edges.data[2 * (edgej + h_firstTypeEdge.data[typej]) + 1]]);
                        p10 = rotate(quatj, p10);
                        p11 = rotate(quatj, p11);

                        m_evaluator.edgeEdge(dx,
                                             p00,
                                             p01,
                                             dx + p10,
                                             dx + p11,
    ...
    int64_t mem_transfer
        = m_pdata->getN() * (5 + 4 + 10) * sizeof(Real) + n_calc * (1 + 3 + 1) * sizeof(Real);
    if (third_law)
        mem_transfer += n_calc * 10 * sizeof(Real);
    if (m_prof)
