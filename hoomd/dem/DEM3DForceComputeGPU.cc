...
DEM3DForceComputeGPU<Real, Real4, Potential>::DEM3DForceComputeGPU(
    ...
    if (!this->m_exec_conf->isCUDAEnabled())
        ...
        this->m_exec_conf->msg->error()
            << "Creating a DEM3DForceComputeGPU with no GPU in the execution configuration" << endl;
    ...
    m_tuner.reset(
        new Autotuner(warp_size, max_threads, warp_size, 5, 100000, "dem_3d", this->m_exec_conf));
    }
...
size_t DEM3DForceComputeGPU<Real, Real4, Potential>::maxGPUThreads() const
    ...
    for (size_t i(0); i < this->m_facesVec.size(); ++i)
        ...
        for (FaceIter faceIter(this->m_facesVec[i].begin()); faceIter != this->m_facesVec[i].end();
             ++faceIter)
            {
...
void DEM3DForceComputeGPU<Real, Real4, Potential>::computeForces(uint64_t timestep)
    ...
    if (third_law)
        {
        this->m_exec_conf->msg->error()
            << "DEM3DForceComputeGPU cannot handle a half neighborlist" << endl;
    ...
    ArrayHandle<Real4> d_pos(this->m_pdata->getPositions(),
                             access_location::device,
                             access_mode::read);
    ArrayHandle<Real4> d_quat(this->m_pdata->getOrientationArray(),
                              access_location::device,
                              access_mode::read);
    ArrayHandle<Scalar> d_diam(this->m_pdata->getDiameters(),
                               access_location::device,
                               access_mode::read);
    ArrayHandle<Real4> d_velocity(this->m_pdata->getVelocities(),
                                  access_location::device,
                                  access_mode::read);

    // GPU array handles
    ArrayHandle<unsigned int> d_nextFaceVert(this->m_nextFaceVert,
                                             access_location::device,
                                             access_mode::read);
    ArrayHandle<unsigned int> d_realVertIndex(this->m_realVertIndex,
                                              access_location::device,
                                              access_mode::read);
    ArrayHandle<unsigned int> d_nextFace(this->m_nextFace,
                                         access_location::device,
                                         access_mode::read);
    ArrayHandle<unsigned int> d_firstFaceVert(this->m_firstFaceVert,
                                              access_location::device,
                                              access_mode::read);
    ArrayHandle<unsigned int> d_firstTypeVert(this->m_firstTypeVert,
                                              access_location::device,
                                              access_mode::read);
    ArrayHandle<unsigned int> d_numTypeVerts(this->m_numTypeVerts,
                                             access_location::device,
                                             access_mode::read);
    ArrayHandle<unsigned int> d_firstTypeEdge(this->m_firstTypeEdge,
                                              access_location::device,
                                              access_mode::read);
    ArrayHandle<unsigned int> d_numTypeEdges(this->m_numTypeEdges,
                                             access_location::device,
                                             access_mode::read);
    ArrayHandle<unsigned int> d_numTypeFaces(this->m_numTypeFaces,
                                             access_location::device,
                                             access_mode::read);
    ArrayHandle<unsigned int> d_vertexConnectivity(this->m_vertexConnectivity,
                                                   access_location::device,
                                                   access_mode::read);
    ArrayHandle<unsigned int> d_edges(this->m_edges, access_location::device, access_mode::read);
    ArrayHandle<Real4> d_verts(this->m_verts, access_location::device, access_mode::read);
    ArrayHandle<Real> d_faceRcutSq(this->m_faceRcutSq, access_location::device, access_mode::read);
    ArrayHandle<Real> d_edgeRcutSq(this->m_edgeRcutSq, access_location::device, access_mode::read);
    ...
    kernel::gpu_compute_dem3d_forces<Real, Real4, DEMEvaluator<Real, Real4, Potential>>(
        ...
        d_quat.data,
        d_nextFace.data,
        d_firstFaceVert.data,
        d_nextFaceVert.data,
        d_realVertIndex.data,
        d_verts.data,
        d_diam.data,
        d_velocity.data,
        (unsigned int)this->maxGPUThreads(),
        (unsigned int)this->maxVertices(),
        (unsigned int)this->numFaces(),
        (unsigned int)this->numDegenerateVerts(),
        (unsigned int)this->numVertices(),
        (unsigned int)this->numEdges(),
        ...
        d_firstTypeVert.data,
        d_numTypeVerts.data,
        d_firstTypeEdge.data,
        d_numTypeEdges.data,
        d_numTypeFaces.data,
        d_vertexConnectivity.data,
        d_edges.data);
