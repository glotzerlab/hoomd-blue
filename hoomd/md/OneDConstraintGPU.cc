...
OneDConstraintGPU::OneDConstraintGPU(std::shared_ptr<SystemDefinition> sysdef,
    ...
    if (!m_exec_conf->isCUDAEnabled())
        ...
        m_exec_conf->msg->error()
            << "Creating a OneDConstraintGPU with no GPU in the execution configuration" << endl;
        throw std::runtime_error("Error initializing OneDConstraintGPU");
        }

    unsigned int warp_size = m_exec_conf->dev_prop.warpSize;
    m_tuner.reset(
        new Autotuner(warp_size, 1024, warp_size, 5, 100000, "oneD_constraint", this->m_exec_conf));
    }
...
void OneDConstraintGPU::computeForces(uint64_t timestep)
    ...
    if (m_prof)
        m_prof->push(m_exec_conf, "OneDConstraint");
...
void export_OneDConstraintGPU(py::module& m)
    {
    py::class_<OneDConstraintGPU, ForceConstraint, std::shared_ptr<OneDConstraintGPU>>(
        m,
        "OneDConstraintGPU")
        .def(
            py::init<std::shared_ptr<SystemDefinition>, std::shared_ptr<ParticleGroup>, Scalar3>());
    }
