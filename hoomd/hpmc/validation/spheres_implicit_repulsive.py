...
class depletion_test(unittest.TestCase):
    ...
    def test_measure_etap(self):
        ...
        log = analyze.log(
            filename=None,
            quantities=['hpmc_overlap_count', 'volume', 'hpmc_free_volume'],
            overwrite=True,
            period=1000)
...
#                print('alpha =', v);
        ...
        if context.current.device.comm.rank == 0:
            ...
            print('tgt: {:.6f} +- {:.6f}'.format(alpha_ref[(phi_c, eta_p_r)][0],
                                                 alpha_ref[(phi_c,
                                                            eta_p_r)][1]))
        ...
        self.assertLessEqual(
            math.fabs(alpha_avg - alpha_ref[(phi_c, eta_p_r)][0]),
            ci * (alpha_ref[(phi_c, eta_p_r)][1] + alpha_err))
