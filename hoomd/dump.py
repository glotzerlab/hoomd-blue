...
class getar(hoomd.analyze._analyzer):
    ...
    **Particle-related metadata**
    ...
    def __init__(self,
        ...
        * '1': One-shot mode: keep only one frame of data. For details on one-shot mode, see the "One-shot mode" section of :py:class:`getar`.
        ...
        if _register:
            ...
            self.analyzer_name = "dump.getar%d" % (
                hoomd.analyze._analyzer.cur_id - 1)

        for val in self._static:
            if prop.name not in self.known_properties:
                raise RuntimeError(
                    'Unknown static property in dump.getar: {}'.format(val))

        for val in self._dynamic:
            if val.name not in self.known_properties:
                raise RuntimeError(
                    'Unknown dynamic property in dump.getar: {}'.format(val))
        ...
        self.cpp_analyzer = _hoomd.GetarDumpWriter(
            hoomd.context.current.system_definition, filename, dumpMode,
            ...
            try:
                if hoomd.context.current.device.comm.num_ranks > 1 and prop.name in self.bad_mpi_properties:
                    ...
                    for period in self._dynamic[prop]:
                        self.cpp_analyzer.setPeriod(
                            ...
                            self.known_resolutions[prop.name],
                            _hoomd.GetarBehavior.Discrete, prop.highPrecision,
                            prop.compression, int(period))
                ...
                if hoomd.context.current.device.comm.num_ranks > 1 and prop.name in self.bad_mpi_properties:
                    ...
                    self.cpp_analyzer.setPeriod(
                        ...
                        self.known_resolutions[prop.name],
                        _hoomd.GetarBehavior.Discrete, prop.highPrecision,
                        prop.compression, int(self._dynamic[prop]))
    ...
    def simple(cls,
        ...
        """
        dynamicDict = {
            cls.DumpProp(name, highPrecision=high_precision): period
            for name in dynamic
        }
    ...
    def immediate(cls, filename, static, dynamic):
        ...
        """
        dumper = getar(filename,
                       'w',
                       static, {key: 1 for key in dynamic},
                       _register=False)
        dumper.cpp_analyzer.analyze(
            hoomd.context.current.system.getCurrentTimeStep())
        dumper.close()
