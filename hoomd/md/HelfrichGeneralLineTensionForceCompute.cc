...
#include "HelfrichGeneralLineTensionForceCompute.h"
#include "BendingRigidityLineTensionForceCompute.h"
...
/*! \file HelfrichGeneralLineTensionForceCompute.cc
    \brief Contains code for the HelfrichGeneralLineTensionForceCompute class
*/
...
*/
HelfrichGeneralLineTensionForceCompute::HelfrichGeneralLineTensionForceCompute(std::shared_ptr<SystemDefinition> sysdef,
                                                   std::shared_ptr<MeshDefinition> meshdef)
    : LineTensionForceCompute(sysdef, meshdef)
    {
    m_exec_conf->msg->notice(5) << "Constructing HelfrichGeneralLineTensionForceCompute" << endl;
...
HelfrichGeneralLineTensionForceCompute::~HelfrichGeneralLineTensionForceCompute()
    {
    m_exec_conf->msg->notice(5) << "Destroying HelfrichGeneralLineTensionForceCompute" << endl;
    }
...
*/
void HelfrichGeneralLineTensionForceCompute::setParams(unsigned int type, const helfrich_param_t& params)
    {
...
void HelfrichGeneralLineTensionForceCompute::setParamsPython(std::string type, pybind11::dict params)
    {
    unsigned int typ = this->m_pdata->getTypeByName(type);
    setParams(typ, helfrich_param_t(params));
    }

pybind11::dict HelfrichGeneralLineTensionForceCompute::getParams(std::string type)
    {
    unsigned int typ = this->m_pdata->getTypeByName(type);
    if (typ >= m_pdata->getNTypes())
        {
        m_exec_conf->msg->error() << "mesh.helfrich: Invalid particle type specified" << endl;
        throw runtime_error("Error setting parameters in HelfrichGeneralLineTensionForceCompute");
        }
    ArrayHandle<helfrich_param_t> h_params(m_params,
                                                             access_location::host,
                                                             access_mode::read);
 ...
 */
void HelfrichGeneralLineTensionForceCompute::computeForces(uint64_t timestep)
    {
    precomputeParameter();
...
void HelfrichGeneralLineTensionForceCompute::precomputeParameter()
    {
    ArrayHandle<Scalar4> h_pos(m_pdata->getPositions(), access_location::host, access_mode::read);
...
Scalar HelfrichGeneralLineTensionForceCompute::energyDiff(unsigned int idx_a,
                                            unsigned int idx_b,
                                            unsigned int idx_c,
                                            unsigned int idx_d,
...
void HelfrichGeneralLineTensionForceCompute::postcomputeParameter(unsigned int idx_a,
                                                    unsigned int idx_b,
                                                    unsigned int idx_c,
                                                    unsigned int idx_d,
                                                    unsigned int type_id)
...
void export_HelfrichGeneralLineTensionForceCompute(pybind11::module& m)
    {
    pybind11::class_<HelfrichGeneralLineTensionForceCompute,
                     LineTensionForceCompute,
                     std::shared_ptr<HelfrichGeneralLineTensionForceCompute>>(m, "HelfrichGeneralLineTensionForceCompute")
        .def(pybind11::init<std::shared_ptr<SystemDefinition>, std::shared_ptr<MeshDefinition>>())
        .def("setParams", &HelfrichGeneralLineTensionForceCompute::setParamsPython)
        .def("getParams", &HelfrichGeneralLineTensionForceCompute::getParams);
    }
