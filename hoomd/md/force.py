...
# on each particle
class Force(hoomd.meta._metadata): ## \internal
    ...
    def __init__(self, name=None):
        ...
        if not hoomd.init.is_initialized(): raise RuntimeError('Cannot create
        force before initialization\n')

        # Allow force to store a name.  Used for discombobulation in the logger
        if name is None: self.name = ""; else: self.name="_" + name

        self.cppForce = None

        # increment the id counter
        id = Force.cur_id; Force.cur_id += 1

        self.force_name = "force%d" % (id); self.enabled = True; self.log =True
    ...
    def disable(self, log=False): R""" Disable the force.
        ...
        if not self.enabled:
            ...
            disable a force that is already disabled"); return

        self.enabled = False; self.log = log

        # remove the compute from the system if it is not going to be logged
        if not log: hoomd.context.current.system.removeCompute(self.force_name)
        hoomd.context.current.forces.remove(self)

    def enable(self): R""" Enable the force.

        Examples::

            force.enable()

        See :py:meth:`disable()`.  """ self.check_initialization()

        # check if we are already disabled
        if self.enabled: hoomd.context.current.device.cpp_msg.warning("Ignoring
        command to enable a force that is already enabled"); return

        # add the compute back to the system if it was removed
        if not self.log: hoomd.context.current.system.addCompute(self.cppForce,
                self.force_name) hoomd.context.current.forces.append(self)

        self.enabled = True; self.log = True

    def get_energy(self,group): R""" Get the energy of a particle group.
        ...
        Examples::
            ...
            g = group.all() energy = force.get_energy(g) """ return
            self.cppForce.calcEnergyGroup(group.cpp_group)

    def get_netForce(self,group): R""" Get the force of a particle group.
    ...
    def get_net_virial(self,group): R""" Get the virial of a particle group.
        ...
        Examples:
            ...
            g = group.all() virial = force.get_net_virial(g) """ return
            np.asarray(self.cppForce.calcVirialGroup(group.cpp_group))
    ...
    def update_coeffs(self): pass raise RuntimeError("Force.update_coeffs should
    not be called")
    ...
    #
    def _Forces(self): return hoomd.data.force_data(self)

    forces = property(_Forces)

    ## \internal
    # \brief Get metadata
    def get_metadata(self): data = hoomd.meta._metadata.get_metadata(self)
    data['enabled'] = self.enabled data['log'] = self.log if self.name is not
    "": data['name'] = self.name
