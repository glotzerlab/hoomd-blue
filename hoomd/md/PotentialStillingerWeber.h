...
/*! \file PotentialStillingerWeber.h
...
/*! <b>Overview:</b>
    PotentialStillingerWeber computes standard three-body potentials and forces between all particles in the
    ...
    EvaluatorStillingerWeber) which is passed in as a template parameter so the computations are performed
    as efficiently as possible.

    PotentialStillingerWeber handles most of the internal details common to all standard three-body
   potentials.
   ...
   are handled
    ...
    In addition, some three-body potentials (such as the
    Stillinger-Weber potential) compute unique forces on
   ...
   appropriate documentation for the evaluator for the definition of each element of the parameters.
    ...
    \sa export_PotentialStillingerWeber()
*/
template<class evaluator> class PotentialStillingerWeber : public ForceCompute
    {
    public:
    ...
    PotentialStillingerWeber(std::shared_ptr<SystemDefinition> sysdef, std::shared_ptr<NeighborList> nlist);
    //! Destructor
    virtual ~PotentialStillingerWeber();
...
PotentialStillingerWeber<evaluator>::PotentialStillingerWeber(std::shared_ptr<SystemDefinition> sysdef,
                                              std::shared_ptr<NeighborList> nlist)
    : ForceCompute(sysdef), m_nlist(nlist), m_typpair_idx(m_pdata->getNTypes())
    {
    this->m_exec_conf->msg->notice(5) << "Constructing PotentialStillingerWeber" << std::endl;
...
template<class evaluator> PotentialStillingerWeber<evaluator>::~PotentialStillingerWeber()
    {
    this->m_exec_conf->msg->notice(5) << "Destroying PotentialStillingerWeber" << std::endl;
    if (m_attached)
        {
        m_nlist->removeRCutMatrix(m_r_cut_nlist);
        }
...
*/
template<class evaluator>
void PotentialStillingerWeber<evaluator>::setParams(unsigned int typ1,
                                            unsigned int typ2,
...
template<class evaluator>
void PotentialStillingerWeber<evaluator>::setParamsPython(pybind11::tuple typ, pybind11::dict params)
    {
...
template<class evaluator> pybind11::dict PotentialStillingerWeber<evaluator>::getParams(pybind11::tuple typ)
    {
    auto typ1 = m_pdata->getTypeByName(typ[0].cast<std::string>());
    auto typ2 = m_pdata->getTypeByName(typ[1].cast<std::string>());
...
*/
template<class evaluator>
void PotentialStillingerWeber<evaluator>::setRcut(unsigned int typ1, unsigned int typ2, Scalar rcut)
    {
...
template<class evaluator>
void PotentialStillingerWeber<evaluator>::setRCutPython(pybind11::tuple types, Scalar r_cut)
    {
...
template<class evaluator>
void PotentialStillingerWeber<evaluator>::validateTypes(unsigned int typ1,
                                                unsigned int typ2,
                                                std::string action)
    {
    ...
    if (typ1 >= n_types || typ2 >= n_types)
        ...
        throw std::runtime_error("Error setting parameters in PotentialStillingerWeber");
        }
    }

template<class evaluator> Scalar PotentialStillingerWeber<evaluator>::getRCut(pybind11::tuple types)
    {
    auto typ1 = m_pdata->getTypeByName(types[0].cast<std::string>());
    auto typ2 = m_pdata->getTypeByName(types[1].cast<std::string>());
...
*/
template<class evaluator> void PotentialStillingerWeber<evaluator>::computeForces(uint64_t timestep)
    {
    // *****  check if we need the structure of the Stillinger-Weber or
    // Tersoff or the RevCross potential for evaluation
    // CD Get rid of the first part of this if???
        ...
        for (int i = 0; i < (int)m_pdata->getN(); i++)
            ...
            for (unsigned int j = 0; j < size; j++)
                ...
                if (evaluated)
                    ...
                    // CD TODO Why is this not used?
                    Scalar bij = Scalar(0.0); // not used
                    // CD This just evaluates the two body part? -
                    // Tersoff doesnt actually have proper three body parameters
                    ...
                    for (unsigned int k = j + 1; k < size;
                        ...
                        if (temp_evaluated)
                            ...
                            // CD Tersoff doesnt actually have proper three
                            // body parameters
                            // CD here we will need to evaluate the
                            // proper 3 body term for SW
    ...
    // CD Up until now is not needed?!?!?!?
        {
        // ****** Tersoff or SquareDensity potential
        // start by updating the neighborlist
        m_nlist->compute(timestep);

        // The three-body potentials can't handle a half neighbor list,
        // so check now.
        // CD What Is this?
        bool third_law = m_nlist->getStorageMode() == NeighborList::half;
        if (third_law)
 ...
 */
template<class evaluator>
CommFlags PotentialStillingerWeber<evaluator>::getRequestedCommFlags(uint64_t timestep)
    {
...
*/
template<class T> void export_PotentialStillingerWeber(pybind11::module& m, const std::string& name)
    {
    pybind11::class_<PotentialStillingerWeber<T>, ForceCompute, std::shared_ptr<PotentialStillingerWeber<T>>>(
        m,
        name.c_str())
        .def(pybind11::init<std::shared_ptr<SystemDefinition>, std::shared_ptr<NeighborList>>())
        .def("setParams", &PotentialStillingerWeber<T>::setParamsPython)
        .def("getParams", &PotentialStillingerWeber<T>::getParams)
        .def("setRCut", &PotentialStillingerWeber<T>::setRCutPython)
        .def("getRCut", &PotentialStillingerWeber<T>::getRCut);
    }
