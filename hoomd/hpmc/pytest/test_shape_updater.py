...
def test_constant_moves(device, simulation_factory, lattice_snapshot_factory):
    ...
    sim.operations.add(mc)
    updater = hoomd.hpmc.update.Alchemy(mc=mc, move_ratio=1.0, seed=3832765, trigger=hoomd.trigger.Periodic(1), nselect=1)
    updater.constant_shape_move(vertices=ConvexPolyhedron(ttf.get_shape(1.0).vertices / (ttf.get_shape(1.0).volume**(1/3))).vertices, ignore_statistics=0, sweep_radius=0.0)
    sim.operations.add(updater)
    sim.operations._schedule()
    sim.run(10)
...
def test_elastic_moves(device, simulation_factory, lattice_snapshot_factory):
    ...
    sim = simulation_factory(lattice_snapshot_factory(dimensions=3, a=2.0, n=4))
    sim.operations.add(mc)
    params = {'mc': mc, 'seed': 1, 'move_ratio': 0.5, 'nselect': 3, 'nsweeps': 2}
    updater = hoomd.hpmc.update.ElasticShape(stepsize=0.001,
                                             stiffness=hoomd.variant.Ramp(1.0, 10.0, 0, 20),
...
def test_tuner(device, simulation_factory, lattice_snapshot_factory):
    ...
    updater.vertex_shape_move(stepsize=0.01, param_ratio=0.2, volume=1.0)
    sim.operations.add(updater)
    tuner = updater.get_tuner(0.2, hoomd.trigger.Periodic(1))
...
def test_logger(device, simulation_factory, lattice_snapshot_factory, tmpdir):
    ...
    sim.operations.add(mc)
    params = {'mc': mc, 'trigger': hoomd.trigger.Periodic(1), 'seed': 1, 'move_ratio': 0.5, 'nselect': 3, 'nsweeps': 2}
    updater = hoomd.hpmc.update.ElasticShape(stepsize=0.001,
                                             stiffness=hoomd.variant.Constant(10.0),
                                             reference=dict(vertices=ConvexPolyhedron(ttf.get_shape(1.0).vertices / (ttf.get_shape(1.0).volume**(1/3))).vertices,
                                                            ignore_statistics=0,
                                                            sweep_radius=0.0),
                                             param_ratio=0.5,
    ...
    for _ in range(10):
        sim.run(1)
        if float(updater.total_count) != 0:
    ...
    np.testing.assert_allclose(volume, np.array([n**3] * len(volume)).reshape(len(volume), 1), rtol=1e-5)
    np.testing.assert_allclose(sp, np.array([0.0] * len(sp)).reshape(len(sp), 1), rtol=1e-5)
    np.testing.assert_allclose(stiffness_arr, np.array([10.0] * len(stiffness_arr)).reshape(len(stiffness_arr), 1), rtol=1e-5)
