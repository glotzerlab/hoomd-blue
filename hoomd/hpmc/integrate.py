    ...
    def __init__(self, seed, d=0.1, a=0.1, move_ratio=0.5, nselect=4, implicit=False, depletant_mode='circumsphere'):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if(implicit):
                # In C++ mode circumsphere = 0 and mode overlap_regions = 1
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphinxUnion(hoomd.context.current.system_definition, seed, 0)
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitSphinxUnion(hoomd.context.current.system_definition, seed, 1)
            else:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoSphinxUnion(hoomd.context.current.system_definition, seed)
        else:
            cl_c = _hoomd.CellListGPU(hoomd.context.current.system_definition);
            hoomd.context.current.system.overwriteCompute(cl_c, "auto_cl2")
            if not implicit:
                self.cpp_integrator = _hpmc.IntegratorHPMCMonoGPUSphinxUnion(hoomd.context.current.system_definition, cl_c, seed)
            else:
                if depletant_mode_circumsphere(depletant_mode):
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitGPUSphinxUnion(hoomd.context.current.system_definition, cl_c, seed)
                else:
                    self.cpp_integrator = _hpmc.IntegratorHPMCMonoImplicitNewGPUSphinxUnion(hoomd.context.current.system_definition, cl_c, seed)
