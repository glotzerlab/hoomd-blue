...
with tag i.
*/
inline void python_list_to_vector_scalar3(const pybind11::list& r0,
    ...
    for (pybind11::ssize_t i = 0; i < n; i++)
        ...
        Scalar x = pybind11::cast<Scalar>(r0_tuple[0]), y = pybind11::cast<Scalar>(r0_tuple[1]),
               z = 0.0;
        if (d == 3)
            {
...
template<class ScalarType> class LatticeReferenceList
    {
    public:
    LatticeReferenceList() : m_N(0) { }

    template<class InputIterator>
    LatticeReferenceList(InputIterator first,
                         InputIterator last,
    ...
    template<class InputIterator>
    void setReferences(InputIterator first,
                       InputIterator last,
                       const std::shared_ptr<ParticleData> pdata,
                       std::shared_ptr<const ExecutionConfiguration> exec_conf)
        {
