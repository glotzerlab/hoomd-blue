 ...
 */
Enforce2DUpdaterGPU::Enforce2DUpdaterGPU(std::shared_ptr<SystemDefinition> sysdef)
    : Enforce2DUpdater(sysdef)
    {
    // at least one GPU is needed
    if (!m_exec_conf->isCUDAEnabled())
        {
        m_exec_conf->msg->error()
            << "Creating a Enforce2DUpdaterGPU with no GPU in the execution configuration" << endl;
...
void Enforce2DUpdaterGPU::update(uint64_t timestep)
    ...
    gpu_enforce2d(m_pdata->getN(), d_vel.data, d_accel.data);
...
void export_Enforce2DUpdaterGPU(py::module& m)
    {
    py::class_<Enforce2DUpdaterGPU, Enforce2DUpdater, std::shared_ptr<Enforce2DUpdaterGPU>>(
        m,
        "Enforce2DUpdaterGPU")
