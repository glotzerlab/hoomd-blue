...
class CompositeSupportFunc2D
    ...
    private:
        ...
        const vec2<OverlapReal>& ab_t;  //!< Vector pointing from a's center to b's center, in the space frame
...
/*! \tparam SupportFuncA Support function class type for shape A
    ...
    are passed in as templated functors. Each functor might store a reference to data (i.e. polygon verts), but the only
    ...
    where particle *A* is at the origin, and particle *B* is at position *ab_t*. Each particle has it's own orientation
    *qa* and *qb*.

    The recommended way of using this code is to specify the support functor in the same file as the shape data
    (e.g. ShapeConvexPolygon.h). Then include XenoCollide2D.h and call xenocollide_2d where needed.
    ...
    **Normalization**
    ...
    This implementation works, but is minimally tested and only on shapes of diameter 1. gjkm_2d() is the production
    code for overlap detection of convex shapes.
...
DEVICE inline bool xenocollide_2d(const SupportFuncA& sa,
    ...
    // In 2D, this step is not necessary (http://xenocollide.snethen.com/mpr2d.html). By choosing the correct normal
    // above, we have a proper portal by construction.
    // The 3D version of this code will need to implement the loop, however. See the book.
    ...
    while(1)
        ...
        // JAA - I originally thought that this was only a tolerance for curved surfaces. However, it is also necessary
        // for polygons. Here is an example edge case where it causes problems:
        // v0 = (-0.0552826,1.3569); v1 = (0.0180616,-0.00930899); v2 = (-0.870821,0.448825); v3 =(-0.870821,0.448825);
        // the portal v1,v2 has narrowed down to the surface of B-A (as evidenced by v3==v2). Thus, the two dots above
        // SHOULD be comparing identical planes and one if should succeed. However, due to truncation error and the fact
        // that v1 is used in the first if and v2 (because v2==v3) in the 2nd, both ifs fail and the code loops
        // infinitely. We can return either a hit or a miss in this case because we can't resolve the difference within
        ...
        // To determine which, construct a line segment from v0 to v3 and test if the origin is on the same side
        // of this line as v1. If so, then the next portal is v3 and v1 - if not, then the next portal is v3 and v2
        v30_perp = perp(v3 - v0);
