...
hipError_t gpu_rattle_nve_step_two(Scalar4 *d_pos,
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        // run the kernel
        hipLaunchKernelGGL((gpu_rattle_nve_step_two_kernel<Manifold>), dim3(grid), dim3(threads), 0, 0, d_pos,
                                                     d_vel,
                                                     d_accel,
                                                     d_group_members,
                                                     nwork,
...
__global__ void gpu_include_rattle_force_nve_kernel(const Scalar4 *d_pos,
    ...
    if (work_idx < nwork)
    	...
    	Scalar3 normal = manifold.derivative(pos); // the normal vector to which the particles are confined.
