#ifndef __POTENTIAL_MESH_BOND_GPU_H__
#define __POTENTIAL_MESH_BOND_GPU_H__
...
/*! \file PotentialMeshBondGPU.h
    \brief Defines the template class for standard mesh_bond potentials on the GPU
...
/*!
    ...
    \tparam gpu_cgbf Driver function that calls gpu_compute_mesh_bond_forces<evaluator>()
...
class PotentialMeshBondGPU : public PotentialMeshBond<evaluator>
    ...
    */
    virtual void setAutotunerParams(bool enable, unsigned int period)
        {
        PotentialMeshBond<evaluator>::setAutotunerParams(enable, period);
...
PotentialMeshBondGPU<evaluator, gpu_cgbf>::PotentialMeshBondGPU(
    ...
    GPUArray<typename evaluator::param_type> params(this->m_mesh_bond_data->getNTypes(),
                                                    this->m_exec_conf);
...
void PotentialMeshBondGPU<evaluator, gpu_cgbf>::computeForces(uint64_t timestep)
    ...
    ArrayHandle<Scalar> d_virial(this->m_virial, access_location::device, access_mode::readwrite);
        ...
        {
        const GPUArray<typename MeshBondData::members_t>& gpu_mesh_bond_list
            = this->m_mesh_bond_data->getGPUTable();
