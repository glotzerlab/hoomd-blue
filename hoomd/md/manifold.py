...
class Diamond(Manifold):
    ...
    Args:
        N (`list` [ `int` ] or `int`): number of unit cells in all 3 directions :math:`[N_x, N_y, N_z]`.
            In case number of unit cells u in all direction the same (:math:`[u, u, u]`), use ``N = u``.
    ...
    * `H. G. von Schnering and R. Nesper 1991  <https://doi.org/10.1007/BF01313411>`_
    ...
    def __preprocess_unitcell(self,value):
        if isinstance(value, Sequence):
            if len(value) != 3:
                raise ValueError(
                    "Expected a single int or three.")
            return tuple(value)
        else:
...
class Primitive(Manifold):
    ...
    Args:
        N (`list` [ `int` ] or `int`): number of unit cells in all 3 directions.
            :math:`[N_{x}, N_{y}, N_{z}]`. In case number of unit cells u in all 
            direction the same (:math:`[u, u, u]`), use ``N = u``.
