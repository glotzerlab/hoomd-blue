    ...
    public:
        ...
        void setReferences(const pybind11::list& r0, const pybind11::list& q0)
            ...
            if (this->m_pdata->getDomainDecomposition())
                ...
                // broadcast particle array size to all processes
                MPI_Bcast(&psz, 1, MPI_UNSIGNED, 0, m_exec_conf->getMPICommunicator());
                if (psz)
                    {
                    if (!pbuffer.size())
                        pbuffer.resize(3*psz, 0.0);
                    // broadcast particle position array to all processes
                    MPI_Bcast(
                            &pbuffer.front(),
                            3*psz,
                            MPI_HOOMD_SCALAR,
                ...
                // broadcast particle orientation array size to all processes
                MPI_Bcast(&qsz, 1, MPI_UNSIGNED, 0, m_exec_conf->getMPICommunicator());
                if (qsz)
                    {
                    if (!qbuffer.size())
                        qbuffer.resize(4*qsz, 0.0);
                    // broadcast orientation array to all processes
