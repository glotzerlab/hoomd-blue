...
@pytest.mark.serial
@pytest.mark.cpu
def test_self_interaction_no_overlap(one_cube_simulation):
    """Check that a properly aligned cube does not overlap itself.

    When aligned to the box, it does not overlap with its own images.
    """
    sim = one_cube_simulation
...
@pytest.mark.serial
@pytest.mark.cpu
def test_self_interaction_overlap(one_cube_simulation):
    """Check that a rotated cube overlaps with itself.
...
def test_self_interaction_run(one_cube_simulation):
