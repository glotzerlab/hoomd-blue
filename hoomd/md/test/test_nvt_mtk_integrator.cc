...
void test_nvt_mtk_integrator(std::shared_ptr<ExecutionConfiguration> exec_conf,
                             twostepnvt_creator nvt_creator)
    {
    ...
    std::shared_ptr<PotentialPairLJ> fc_1
        = std::shared_ptr<PotentialPairLJ>(new PotentialPairLJ(sysdef_1, nlist_1));
    ...
    std::shared_ptr<ComputeThermo> thermo_nvt
        = std::shared_ptr<ComputeThermo>(new ComputeThermo(sysdef_1, group_all_1));
    ...
    std::cout << "Measuring temperature and conserved quantity for another 25,000 time steps..."
              << std::endl;
    Scalar avg_T(0.0);
...
void test_nvt_mtk_integrator_aniso(std::shared_ptr<ExecutionConfiguration> exec_conf,
                                   twostepnvt_creator nvt_creator)
    {
    ...
    std::shared_ptr<AnisoPotentialPair<EvaluatorPairGB>> fc_1
        = std::shared_ptr<AnisoPotentialPair<EvaluatorPairGB>>(new AnisoPotentialPair<EvaluatorPairGB>(sysdef_1, nlist_1));
    ...
    int n_measure_steps = 25000;
    std::cout << "Measuring temperature and conserved quantity for another " << n_measure_steps
              << " time steps..." << std::endl;
    Scalar avg_T(0.0);
...
void nvt_updater_compare_test(twostepnvt_creator nvt_creator1,
                              twostepnvt_creator nvt_creator2,
...
UP_TEST(TwoStepNVTMTK_basic_test)
    {
    test_nvt_mtk_integrator(std::shared_ptr<ExecutionConfiguration>(
                                new ExecutionConfiguration(ExecutionConfiguration::CPU)),
                            bind(base_class_nvt_creator, _1, _2, _3, _4, _5));
    }

//! Performs a basic equilibration test of TwoStepNVTMTK
UP_TEST(TwoStepNVTMTK_basic_aniso_test)
    {
    test_nvt_mtk_integrator_aniso(std::shared_ptr<ExecutionConfiguration>(
                                      new ExecutionConfiguration(ExecutionConfiguration::CPU)),
                                  bind(base_class_nvt_creator, _1, _2, _3, _4, _5));
    }
...
UP_TEST(TwoStepNVTMTKGPU_basic_test)
    ...
    test_nvt_mtk_integrator(std::shared_ptr<ExecutionConfiguration>(
                            ...
                            bind(gpu_nvt_creator, _1, _2, _3, _4, _5));
    }
...
UP_TEST(TwoStepNVTMTKGPU_comparison_tests)
    ...
    nvt_updater_compare_test(nvt_creator,
                             nvt_creator_gpu,
