...
hipError_t gpu_rattle_brownian_step_one(Scalar4 *d_pos,
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        // run the kernel
        hipLaunchKernelGGL((gpu_rattle_brownian_step_one_kernel<Manifold>), dim3(grid), dim3(threads), (unsigned int)(sizeof(Scalar)*rattle_bd_args.n_types + sizeof(Scalar3)*rattle_bd_args.n_types), 0, d_pos,
                                     d_image,
                                     d_vel,
                                     box,
                                     d_diameter,
