...
*/
__global__ void gpu_compute_shear_force_set_forces_kernel(const unsigned int group_size,
    ...
    vec3<Scalar> fi(posidx.y*s_f, 0, 0);
...
hipError_t gpu_compute_shear_force_set_forces(const unsigned int group_size,
    ...
    // run the kernel
    hipLaunchKernelGGL((gpu_compute_shear_force_set_forces_kernel),
                       dim3(grid),
                       dim3(threads),
                       0,
                       0,
