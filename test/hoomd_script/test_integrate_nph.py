...
class integrate_nph_tests (unittest.TestCase):
    ...
    def test(self):
        ...
        integrate.nph(all, T=1.2, tau=0.5, P=1.0, tauP=0.5);
        run(100);
    ...
    def test_mtk_orthorhombic(self):
        all = group.all();
        integrate.mode_standard(dt=0.005);
        integrate.nph(all, T=1.2, tau=0.5, P=1.0, tauP=0.5, couple="none");
        run(100);

    def test_mtk_tetragonal(self):
        all = group.all();
        integrate.mode_standard(dt=0.005);
        integrate.nph(all, T=1.2, tau=0.5, P=1.0, tauP=0.5, couple="xy");
        run(100);

    def test_mtk_triclinic(self):
        all = group.all();
        integrate.mode_standard(dt=0.005);
        integrate.nph(all, T=1.2, tau=0.5, P=1.0, tauP=0.5, couple="none", all=True);
        run(100);
    ...
    def test_empty(self):
        ...
        nve = integrate.nph(group=empty, T=1.0, P=1.0, tau=0.5, tauP=0.5,mtk=False)
        run(1);
    ...
    def test_empty_mtk(self):
        empty = group.cuboid(name="empty", xmin=-100, xmax=-100, ymin=-100, ymax=-100, zmin=-100, zmax=-100)
        mode = integrate.mode_standard(dt=0.005);
        nve = integrate.nph(group=empty, T=1.0, P=1.0, tau=0.5, tauP=0.5)
        run(1);
