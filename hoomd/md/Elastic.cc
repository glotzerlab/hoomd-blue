...
void Elastic::setReference(pybind11::array_t<Scalar> reference_positions, 
	...
	for(unsigned int i=0; i < n_tetrahedra; i++){
		...
		for(unsigned int column=0; column<3; column++){
				reference_vertex_displacement_matrix(0,column) = h_reference_vertex_displacements.data[m_matrix_indexer(i,column)].x;
				reference_vertex_displacement_matrix(1,column) = h_reference_vertex_displacements.data[m_matrix_indexer(i,column)].y;
				reference_vertex_displacement_matrix(2,column) = h_reference_vertex_displacements.data[m_matrix_indexer(i,column)].z;
		}	

		auto reference_vertex_displacement_matrix_inverse = reference_vertex_displacement_matrix.inverse();

		for(unsigned int column=0; column<3; column++){
			h_reference_inv_matrix.data[m_matrix_indexer(i,column)].x = reference_vertex_displacement_matrix_inverse(0,column);
		    h_reference_inv_matrix.data[m_matrix_indexer(i,column)].y = reference_vertex_displacement_matrix_inverse(1,column);
			h_reference_inv_matrix.data[m_matrix_indexer(i,column)].z = reference_vertex_displacement_matrix_inverse(2,column);
		}
...
void Elastic::computeForces(uint64_t timestep){
    ...
    for(unsigned int i=0; i < n_tetrahedra; i++){
        ...
        vec3<Scalar> pos0 = vec3<Scalar>(h_pos.data[idx_0].x, h_pos.data[idx_0].y, h_pos.data[idx_0].z);
        vec3<Scalar> pos1 = vec3<Scalar>(h_pos.data[idx_1].x, h_pos.data[idx_1].y, h_pos.data[idx_1].z);
        vec3<Scalar> pos2 = vec3<Scalar>(h_pos.data[idx_2].x, h_pos.data[idx_2].y, h_pos.data[idx_2].z);
        vec3<Scalar> pos3 = vec3<Scalar>(h_pos.data[idx_3].x, h_pos.data[idx_3].y, h_pos.data[idx_3].z);
		...
		vec3<Scalar> reference_vertex_displacement_0 = h_reference_vertex_displacements.data[m_matrix_indexer(i,0)];
		vec3<Scalar> reference_vertex_displacement_1 = h_reference_vertex_displacements.data[m_matrix_indexer(i,1)];
		vec3<Scalar> reference_vertex_displacement_2 = h_reference_vertex_displacements.data[m_matrix_indexer(i,2)];

		vec3<Scalar> u = vec3<Scalar>(
						vertex_displacement_0.x - reference_vertex_displacement_0.x,
						vertex_displacement_1.x - reference_vertex_displacement_1.x,
						vertex_displacement_2.x - reference_vertex_displacement_2.x);

		vec3<Scalar> v = vec3<Scalar>(
						vertex_displacement_0.y - reference_vertex_displacement_0.y,
						vertex_displacement_1.y - reference_vertex_displacement_1.y,
						vertex_displacement_2.y - reference_vertex_displacement_2.y);

		vec3<Scalar> w = vec3<Scalar>(
						vertex_displacement_0.z - reference_vertex_displacement_0.z,
		...
		vec3<Scalar> a = vec3<Scalar> (dot(inverse_matrix_row_0, u), dot(inverse_matrix_row_1, u), dot(inverse_matrix_row_2, u));
		vec3<Scalar> b = vec3<Scalar> (dot(inverse_matrix_row_0, v), dot(inverse_matrix_row_1, v), dot(inverse_matrix_row_2, v));
		vec3<Scalar> c = vec3<Scalar> (dot(inverse_matrix_row_0, w), dot(inverse_matrix_row_1, w), dot(inverse_matrix_row_2, w));

        // Step 3: Calculate Strains (epsilons)
		
		Scalar strain_tensor[3][3] = {
			{0,0,0},
			{0,0,0},
			{0,0,0}
		};	
