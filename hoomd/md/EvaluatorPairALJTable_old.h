...
#define HOOMD_ALJTABLE_MIN(i,j) ((i > j) ? j : i)
#define HOOMD_ALJTABLE_MAX(i,j) ((i > j) ? i : j)
        ...
        */
        ...
        evaluate(Scalar3& force, Scalar& pair_eng, bool energy_shift, Scalar3& torque_i, Scalar3& torque_j)
            ...
            if ( (r/_params.ki_max < sqrt(rcutsq)) | (r/_params.kj_max < sqrt(rcutsq)) )
              ...
              //if (!success)
              //{
              //  gjk<3>(vertsj, _params.Nj, vertsi, _params.Ni, v, a, b, success);
              //}

              // If nan from gjk...fallback on slower gjk
              //if ( isnan(dot(v,v)) | (dot(b-a,b-a) < 1.0E-4) | isnan(fast::rsqrt(dot(v,v))) )
              //  {
                //////////////////////////////
                // Call gjk -- Thi version //
                //////////////////////////////
              //  vec3<Scalar> GG;
              //  vec3<Scalar> HH;
              //  Scalar rtmp = gjk_distance(xi_final, xj_final, yi_final, yj_final, zi_final, zj_final, _params.Ni, _params.Nj, Scalar(-1.0)*unitr, GG, HH);              
              //  v = HH - GG;
              //  b = HH;
              //  a = GG;
                //////////////////////////////
...
//                }              
              ...
              if (!overlap)
                ...
                // // Overlap: sum over overlap points
                ...
                // f_scalar_contact2 = 4.0*epsilon;

                // // Net force
                // Scalar factor_test = 1.0;
                // f = f_scalar * unitr - factor_test*Scalar(0.5)*( f_scalar_contact1 * rvect1 - f_scalar_contact2 * rvect2 );              
                // force = vec_to_scalar3(f);

                // // Torque
                // torque_i = vec_to_scalar3(cross(rvect1*r1check-(b-a),factor_test*Scalar(-1.0)*(f_scalar_contact1*rvect1 - f_scalar_contact2*rvect2)));
                // torque_j = vec_to_scalar3(cross(rvect1*r1check-Scalar(-1.0)*dr,factor_test*Scalar(1.0)*(f_scalar_contact1*rvect1 - f_scalar_contact2*rvect2))); 
...
#undef HOOMD_ALJTABLE_MIN
#undef HOOMD_ALJTABLE_MAX
