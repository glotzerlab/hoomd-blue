...
@pytest.fixture
def make_two_particle_simulation(two_particle_snapshot_factory, simulation_factory):
    def make_simulation(force, d=1, types=None, dimensions=3):
        ...
        if snap.communicator.rank == 0:
            ...
            snap.particles.moment_inertia[1] = [1.0, 2.0, 1.0]
...
def test_mode_invalid(mode):
    ...
    with pytest.raises(TypeConversionError):
        gay_berne = md.pair.aniso.GayBerne(
            nlist=md.nlist.Cell(buffer=0.4), default_r_cut=2.5, mode=mode
        )
    gay_berne = md.pair.aniso.GayBerne(
...
@pytest.mark.parametrize("r_cut", [-1.0, "foo", None])
...
AnisoPotentialSpecification = namedtuple(
    "AnisoParametersSpecification", ("cls", "type_parameters")
)
...
def _valid_params(particle_types=["A", "B"]):
    ...
    def to_type_parameter_dicts(types, argument_dict):
        ...
        for name, (values, num_types) in argument_dict.items():
            ...
            if isinstance(values, Mapping):
                ...
                tp_spec = {type_key: spec for type_key, spec in zip(type_keys, values)}
    ...
    dipole_arg_dict = {
        "params": ({"A": [0.5, 1.5, 3.47], "kappa": [4.0, 1.2, 0.3]}, 2),
    ...
    valid_params_list.append(
        make_aniso_spec(
            md.pair.aniso.ALJ, to_type_parameter_dicts(particle_types, alj_arg_dict0)
        )
    )
    ...
    valid_params_list.append(
        make_aniso_spec(
            md.pair.aniso.ALJ, to_type_parameter_dicts(particle_types, alj_arg_dict1)
        )
    )
...
@pytest.mark.parametrize("pair_potential_spec", _valid_params(), ids=PotentialId())
def test_setting_params_and_shape(make_two_particle_simulation, pair_potential_spec):
...
def _aniso_forces_and_energies():
    ...
    with path.open() as f:
        ...
        for pot in computations:
            for i, params in enumerate(expand_dict(computations[pot]["params"])):
                fet_list.append(
                    FETtuple(
...
def test_run(simulation_factory, lattice_snapshot_factory, pair_potential):
    snap = lattice_snapshot_factory(particle_types=["A", "B"], n=7, a=2.0, r=0.01)
    if snap.communicator.rank == 0:
    ...
    autotuned_kernel_parameter_check(
        instance=pair_potential, activate=lambda: sim.run(1)
    )
...
def test_aniso_force_computes(make_two_particle_simulation, aniso_forces_and_energies):
    ...
    for i, (distance, orientation) in enumerate(zip(particle_distances, orientations)):
        ...
        if sim_energies is not None:
            assert isclose(sim_energies[0], aniso_forces_and_energies.energies[i])
...
def _base_expected_loggable(include_type_shapes=False):
    ...
    if include_type_shapes:
        base["type_shapes"] = {"category": LoggerCategories.object, "default": True}
    return base
...
@pytest.mark.parametrize(
    ...
    (
        (cls, log_check_params)
        for cls, log_check_params in zip(
            (md.pair.aniso.GayBerne, md.pair.aniso.Dipole, md.pair.aniso.ALJ),
            (
