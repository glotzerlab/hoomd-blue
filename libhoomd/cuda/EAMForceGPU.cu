...
extern "C" __global__ void gpu_compute_eam_forces_kernel(
			...
			if (rsq < eam_data.r_cutsq)
				{
				 ...
				 position -= r_index;				 
				 atomElectronDensity += electronDensity[r_index + eam_data.nr * typej] + derivativeElectronDensity[r_index + eam_data.nr * typej] * position * eam_data.dr;		 
				}
			}
	...
	force.w += embeddingFunction[r_index + typei * eam_data.nrho] + derivativeEmbeddingFunction[r_index + typei * eam_data.nrho] * position * eam_data.drho;
	...
	for (int neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++)
		...
		if (neigh_idx < n_neigh)
			...
			if (rsq < eam_data.r_cutsq)
				...
				float pair_eng = (pairPotential[r_index + (typei + typej) * eam_data.nr] + 
					derivativePairPotential[r_index + (typei + typej) * eam_data.nr] * position * eam_data.dr) * inverseR;
				float derivativePhi = (derivativePairPotential[r_index + (typei + typej) * eam_data.nr] - pair_eng) * inverseR;
				float derivativeRhoI = derivativeElectronDensity[r_index + typei * eam_data.nr];			
				float derivativeRhoJ = derivativeElectronDensity[r_index + typej * eam_data.nr];			
