...
def get_mesh_forces_and_energies():
    ...
    for i in range(7):
        args_and_vals.append((forces[i], energies[i], height[i], angle[i], radius[i]))
...
def test_before_attaching():
    ...
    tlj = hoomd.md.mesh.triangle_pair.WCA(
        nlist=tree, mesh=mesh, default_r_cut=2 ** (1 / 6), default_nlist_r_cut=5
    )
...
def test_after_attaching(
    ...
    tlj = hoomd.md.mesh.triangle_pair.WCA(
        nlist=tree, mesh=mesh, default_r_cut=0, default_nlist_r_cut=5
    )
    tlj.params["A"] = dict(epsilon=2, sigma=1)
