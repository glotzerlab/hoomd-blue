...
DEVICE inline void DEMEvaluator<Real, Real4, Potential>::comCOM(const Vec& rij,
                                                                Real& potential,
                                                                Vec& force_i,
                                                                Torque& torque_i,
                                                                Vec& force_j,
                                                                Torque& torque_j) const
    {
...
template<typename Real, typename Real4, typename Potential>
template<typename Vec, typename Torque>
DEVICE inline void DEMEvaluator<Real, Real4, Potential>::vertexVertex(const Vec& rij,
                                                                      const Vec& r0,
                                                                      const Vec& r1,
                                                                      Real& potential,
                                                                      Vec& force_i,
...
template<typename Real, typename Real4, typename Potential>
template<typename Vec, typename Torque>
DEVICE inline void DEMEvaluator<Real, Real4, Potential>::vertexEdge(const Vec& rij,
                                                                    const Vec& r0,
                                                                    const Vec& r1,
                                                                    const Vec& r2,
                                                                    Real& potential,
    ...
    m_potential
        .evaluate(rij, r0, rPrime, potential, force_i, torque_i, force_j, torque_j, modFactor);
    }

template<typename Real, typename Real4, typename Potential>
DEVICE inline void
DEMEvaluator<Real, Real4, Potential>::vertexFace(const vec3<Real>& rij,
                                                 const vec3<Real>& r0,
                                                 const quat<Real> quatj,
                                                 const Real4* verticesj,
                                                 const unsigned int* realIndicesj,
                                                 const unsigned int* facesj,
                                                 const unsigned int vertex0Index,
                                                 Real& potential,
                                                 vec3<Real>& force_i,
                                                 vec3<Real>& torque_i,
                                                 vec3<Real>& force_j,
                                                 vec3<Real>& torque_j) const
    {
    ...
    const vec3<Real> secondVertex(
        rotate(quatj, vec3<Real>(verticesj[realIndicesj[facesj[vertex0Index]]])));
    ...
    if (distsq > 0)
        m_potential
            .evaluate(rij, r0, rPrime + rij, potential, force_i, torque_i, force_j, torque_j);
    }

// convenience function for edge/edge calculation
template<typename Real>
DEVICE inline Real
detp(const vec3<Real>& m, const vec3<Real>& n, const vec3<Real> o, const vec3<Real> p)
    {
    return dot(m - n, o - p);
    }

template<typename Real, typename Real4, typename Potential>
DEVICE inline void DEMEvaluator<Real, Real4, Potential>::edgeEdge(const vec3<Real>& rij,
                                                                  const vec3<Real>& p00,
                                                                  const vec3<Real>& p01,
                                                                  const vec3<Real>& p10,
                                                                  const vec3<Real>& p11,
    ...
    Real lambda0((detp(p00, p10, p11, p10) * detp(p11, p10, p01, p00)
                  - detp(p00, p10, p01, p00) * detp(p11, p10, p11, p10))
                 / denominator);
    Real lambda1((detp(p00, p10, p11, p10) + lambda0 * detp(p11, p10, p01, p00))
                 / detp(p11, p10, p11, p10));
