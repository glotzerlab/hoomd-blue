...
void CellListStencil::compute(uint64_t timestep)
    ...
    unsigned int max_n_stencil = (2 * max_stencil_size.x + 1) * (2 * max_stencil_size.y + 1)
                                 * (2 * max_stencil_size.z + 1);
    ...
    // the cell in the "middle" of the box (will be used to guard against running over ends or
    // double counting)
    ...
    for (unsigned int cur_type = 0; cur_type < m_pdata->getNTypes(); ++cur_type)
        ...
        h_stencil.data[m_stencil_idx(0, cur_type)]
            = make_scalar4(__int_as_scalar(0), __int_as_scalar(0), __int_as_scalar(0), 0.0);
        unsigned int n_stencil_i = 1;
        for (int k = -stencil_size.z; k <= stencil_size.z; ++k)
            {
            // skip this stencil site if it could take the representative "origin" stencil out of
            // the grid by symmetry of the stencil and because of periodic wrapping. this stops us
            // from double counting cases that would cover the entire grid
            if (periodic.z && ((origin.z + k) < 0 || (origin.z + k) >= (int)dim.z))
                continue;

            for (int j = -stencil_size.y; j <= stencil_size.y; ++j)
                {
                if (periodic.y && ((origin.y + j) < 0 || (origin.y + j) >= (int)dim.y))
                    continue;

                for (int i = -stencil_size.x; i <= stencil_size.x; ++i)
                    {
                    if (periodic.z && ((origin.x + i) < 0 || (origin.x + i) >= (int)dim.x))
                        continue;
                    ...
                    if (i > 0)
                        dr.x = (i - 1) * cell_size.x;
                    else if (i < 0)
                        dr.x = (i + 1) * cell_size.x;

                    if (j > 0)
                        dr.y = (j - 1) * cell_size.y;
                    else if (j < 0)
                        dr.y = (j + 1) * cell_size.y;

                    if (k > 0)
                        dr.z = (k - 1) * cell_size.z;
                    else if (k < 0)
                        dr.z = (k + 1) * cell_size.z;
                    ...
                    if (dr2 < r_listsq_max)
                        {
                        h_stencil.data[m_stencil_idx(n_stencil_i, cur_type)]
                            = make_scalar4(__int_as_scalar(i),
                                           __int_as_scalar(j),
                                           __int_as_scalar(k),
                                           dr2);
                        ++n_stencil_i;
