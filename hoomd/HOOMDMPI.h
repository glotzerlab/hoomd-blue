...
class GatherTagOrder
    ...
    public:
        ...
        */
        ...
        GatherTagOrder(const MPI_Comm mpi_comm=MPI_COMM_WORLD, int root=0) : m_mpi_communicator(mpi_comm),
        m_root(root)
            {
            int rank;
            int n_ranks;
        ...
        void setLocalTagsSorted(unsigned int *local_tag, int n_local_tags)
            ...
            MPI_Gather(&n_local_tags, 1, MPI_INT, m_recv_counts.data(), 1, MPI_INT, m_root, m_mpi_communicator);

            unsigned int *gathered_tags = nullptr;
            if (rank == m_root)
                {
                std::exclusive_scan(m_recv_counts.begin(), m_recv_counts.end(), m_displacements.begin(), 0);
            ...
            if (rank == m_root)
                ...
                for (int i = 0; i < n_ranks; i++)
                    ...
                    if (m_recv_counts[i] > 0)
                        {
                        m_sort_queue.push(std::make_tuple(gathered_tags[m_displacements[i]],
                                                               i,
                                                               0));
                        }
                    }
                ...
                while (!m_sort_queue.empty())
                    ...
                    if (local_index < static_cast<unsigned int>(m_recv_counts[item_rank]))
                        {
                        m_sort_queue.push(std::make_tuple(gathered_tags[global_index + 1],
                                                               item_rank,
                                                               local_index));
                        }
        ...
        void gatherArray(std::vector<T>& global_array, const T* local_array, int local_size)
            ...
            if (rank == m_root)
                ...
                std::transform(m_recv_counts.begin(), m_recv_counts.end(),
                               m_recv_bytes.begin(),
                               [](int v) { return v * sizeof(T); });
                std::transform(m_displacements.begin(), m_displacements.end(),
    ...
    private:
        ...
        std::priority_queue<queue_item,
        std::vector<queue_item>, std::greater<queue_item>> m_sort_queue;
