...
def hoomd_snapshot(lattice_snapshot_factory):
    ...
    if snap.communicator.rank == 0:
        ...
        snap.particles.velocity[:] = np.tile(
            np.linspace(1, 2, 3), (snap.particles.N, 1)
        )
        snap.particles.acceleration[:] = np.tile(
            np.linspace(1, 2, 3), (snap.particles.N, 1)
        )
...
def test_write_gsd_mode(create_md_sim, hoomd_snapshot, tmp_path, simulation_factory):
    ...
    if sim.device.communicator.num_ranks == 1:
        ...
        with pytest.raises(Exception, match=r".*File exists.*"):
            sim.operations.writers.append(gsd_writer)
            sim.run(1)
...
def test_write_gsd_truncate(create_md_sim, tmp_path):
    ...
    gsd_writer = hoomd.write.GSD(
        filename=filename, trigger=hoomd.trigger.Periodic(1), truncate=True, mode="wb"
    )
    sim.operations.writers.append(gsd_writer)
...
def test_write_gsd_dynamic(simulation_factory, create_md_sim, tmp_path):
    ...
    gsd_writer = hoomd.write.GSD(
        filename=filename, trigger=hoomd.trigger.Periodic(1), mode="wb"
    )
    sim.operations.writers.append(gsd_writer)
    velocity_list = []
    position_list = []
        ...
        with gsd.hoomd.open(name=filename, mode="r") as traj:
            for step in range(5):
                np.testing.assert_allclose(
                    traj[step].particles.position,
                    position_list[step],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
                np.testing.assert_allclose(
                    traj[step].particles.orientation,
                    N_particles * [[1, 0, 0, 0]],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
                np.testing.assert_allclose(
                    traj[step].particles.velocity,
                    velocity_list[0],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
    ...
    if sim.device.communicator.rank == 0:
        with gsd.hoomd.open(name=filename, mode="r") as traj:
            for step in range(5):
                ...
                np.testing.assert_allclose(
                    traj[step].particles.angmom,
                    angmom_list[step],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
                np.testing.assert_allclose(
                    traj[step].particles.image,
                    N_particles * [[0, 0, 0]],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
    ...
    if sim.device.communicator.rank == 0:
        with gsd.hoomd.open(name=filename, mode="r") as traj:
            for step in range(5, 10):
                np.testing.assert_allclose(
                    traj[step].particles.mass,
                    N_particles * [0.8],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
                np.testing.assert_allclose(
                    traj[step].particles.charge,
                    N_particles * [0],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
                np.testing.assert_allclose(
                    traj[step].particles.body,
                    N_particles * [-1],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
                np.testing.assert_allclose(
                    traj[step].particles.moment_inertia,
                    N_particles * [[0, 0, 0]],
                    rtol=1e-07,
                    atol=1.5e-07,
                )
...
def test_write_gsd_finegrained_dynamic(
    simulation_factory, hoomd_snapshot, tmp_path, dynamic_field
):
    ...
    if hoomd_snapshot.communicator.rank == 0:
        ...
        hoomd_snapshot.particles.typeid[:] = np.tile(1, hoomd_snapshot.particles.N)
        hoomd_snapshot.particles.mass[:] = np.tile(2, hoomd_snapshot.particles.N)
        hoomd_snapshot.particles.diameter[:] = np.tile(4, hoomd_snapshot.particles.N)
        hoomd_snapshot.particles.body[:] = np.tile(4, hoomd_snapshot.particles.N)
    ...
    if sim.device.communicator.rank == 0:
        with gsd.fl.open(name=filename, mode="r") as f:
            ...
            np.testing.assert_allclose(
                data,
                getattr(hoomd_snapshot.particles, dynamic_field[10:]),
                rtol=1e-07,
                atol=1.5e-07,
            )
...
@pytest.mark.parametrize("dynamic_field", dynamic_fields)
def test_write_gsd_finegrained_dynamic_alldefault(
    simulation_factory, hoomd_snapshot, tmp_path, dynamic_field
):
    filename = tmp_path / "test_finegrained_dynamic.gsd"
    ...
    if hoomd_snapshot.communicator.rank == 0:
        ...
        hoomd_snapshot.particles.typeid[:] = np.tile(0, hoomd_snapshot.particles.N)
        hoomd_snapshot.particles.charge[:] = np.tile(0, hoomd_snapshot.particles.N)
...
def test_write_gsd_no_dynamic(simulation_factory, hoomd_snapshot, tmp_path):
    ...
    gsd_writer = hoomd.write.GSD(
        filename=filename, trigger=hoomd.trigger.Periodic(1), mode="wb", dynamic=[]
    )
    sim.operations.writers.append(gsd_writer)
