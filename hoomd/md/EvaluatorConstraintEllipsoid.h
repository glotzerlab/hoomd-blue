...
/*! <b>General Overview</b>
    EvaluatorConstraintEllipsoid is a low level computation helper class to aid in evaluating
   particle constraints on a ellipsoid. Given a ellipsoid at a given position and radii, it will
   find the nearest point on the Ellipsoid to a given position.
*/
class EvaluatorConstraintEllipsoid
    ...
    */
    ...
    //! Evaluate the closest point on the ellipsoid. Method from:
    ...
    DEVICE Scalar3 evalClosest(const Scalar3& U)
        ...
        if (rx == ry && ry == rz) // if ellipsoid is actually a sphere, use easier method
            ...
            if (g != 0) // point does not lay on ellipsoid
                ...
                if (g > 0)
                    {
                    s1 = slow::sqrt(n0 * n0 + n1 * n1 + z2 * z2) - 1;
                    }
                Scalar sbar;
                Scalar ratio0, ratio1, ratio2;
                int i, imax = 10000; // When tested, on average takes ~30 steps to complete, and
                                     // never more than 150.
                for (i = 0; i < imax; i++)
                    {
                    sbar = (s0 + s1) / 2.0;
                ...
                // if (i == imax)
                // {
                //     throw runtime_error("constrain.ellipsoid: Not enough iteration steps to find
