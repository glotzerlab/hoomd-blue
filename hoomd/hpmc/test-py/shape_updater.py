...
class shape_updater_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.system = create_empty(N=8,
                                   box=hoomd.data.boxdim(L=5),
    ...
    def test_sphere_python(self):
        ...
        with self.assertRaises(RuntimeError):
            self.updater = hpmc.update.alchemy(mc=self.mc,
                                               move_ratio=1.0,
                                               seed=3832765,
                                               period=1,
                                               nselect=1)
        ...
        with self.assertRaises(RuntimeError):
            self.updater.python_shape_move(callback=fun,
                                           params=p,
                                           stepsize=0.0,
                                           param_ratio=1.0)
            hoomd.run(10, quiet=True)
    ...
    def test_sphere_elastic(self):
        ...
        self.mc.shape_param.set(self.types, diameter=diam)
        with self.assertRaises(RuntimeError):
            self.updater = hpmc.update.elastic_shape(
                mc=self.mc,
                stepsize=0.001,
                move_ratio=1.0,
                seed=3832765,
                stiffness=10.0,
                reference=dict(diameter=diam),
                nsweeps=2,
                param_ratio=0.5)
    ...
    def test_convex_polyhedron_vertex(self):
        ...
        self.updater = hpmc.update.alchemy(mc=self.mc,
                                           ...
                                           nselect=1)
        hoomd.run(10, quiet=True)
        self.updater.vertex_shape_move(stepsize=0.01,
                                       param_ratio=0.2,
                                       volume=1.0)
        hoomd.run(10, quiet=True)
    ...
    def test_convex_polyhedron_elastic(self):
        ...
        self.mc.shape_param.set(self.types, vertices=v)
        self.pos = None
        #hoomd.deprecated.dump.pos("shape_updater.pos", period=10)
        self.updater = hpmc.update.elastic_shape(mc=self.mc,
                                                 stepsize=0.001,
                                                 move_ratio=1.0,
                                                 seed=3832765,
                                                 stiffness=1.0,
                                                 reference=dict(vertices=v),
                                                 pos=self.pos,
                                                 nselect=3,
                                                 nsweeps=2,
                                                 param_ratio=0.5)
    ...
    def test_tuner_average(self):
        ...
        for _ in range(100):
            print(
                "stepsizes: ",
                [self.updater.get_step_size(i) for i in range(len(self.types))])
            print("acceptance: ", [
                self.updater.get_move_acceptance(i)
    ...
    def test_logger(self):
        ...
        hoomd.analyze.log(filename="shape_updater.log",
                          period=10,
                          overwrite=True,
                          quantities=[
                              'shape_move_energy', 'shape_move_stiffness',
                              'shape_move_acceptance_ratio',
                              'shape_move_particle_volume'
                          ])
        hoomd.run(101)

    def test_iq_logging(self):
        v = np.array(
            [[x, y, z] for x, y, z in itertools.product((-0.5, 0.5), repeat=3)])
        ...
        self.updater = hpmc.update.alchemy(mc=self.mc,
                                           ...
                                           period=1,
                                           nselect=1)
        self.updater.vertex_shape_move(stepsize=0.0,
                                       param_ratio=0.2,
                                       volume=1.0)
        log1, log2 = 'shape_isoperimetric_quotient_1', 'shape_isoperimetric_quotient'
        iq_logger = hoomd.analyze.log(filename="iq.log",
                                      period=1,
                                      overwrite=True,
