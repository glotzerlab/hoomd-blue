...
namespace hoomd::md {
    ...
    class Thermostat {
    public:
        Thermostat(std::shared_ptr<ParticleGroup> group,
                   std::shared_ptr<ComputeThermo> thermo,
                   std::shared_ptr<Variant> T) :
        m_group(group), m_thermo(thermo), m_T(T)
        {
        }

        virtual ~Thermostat() {}

        virtual std::array<Scalar, 2> RescalingFactors_one(uint64_t timestep, Scalar deltaT) {
            return {Scalar(1.0), Scalar(1.0)};
        }

        virtual std::array<Scalar, 2> RescalingFactors_two(uint64_t timestep, Scalar deltaT) {
            return {Scalar(1.0), Scalar(1.0)};
        }
        ...
        virtual void setTranslationalDOF(pybind11::tuple){}

        virtual pybind11::tuple getTranslationalDOF(){
            return pybind11::make_tuple();
        }

        virtual void setRotationalDOF(pybind11::tuple){}

        virtual pybind11::tuple getRotationalDOF(){
            return pybind11::make_tuple();
        };

        Scalar getThermostatEnergy(){return Scalar(0.0);}
    ...
    public:
        MTTKThermostat(std::shared_ptr<ParticleGroup> group,
                       std::shared_ptr<ComputeThermo> thermo,
                       std::shared_ptr<Variant> T,
                       Scalar tau) :
                Thermostat(group, thermo, T), m_tau(tau) {}
        ...
        std::array<Scalar, 2> RescalingFactors_one(uint64_t timestep, Scalar deltaT) override{
        ...
        std::array<Scalar, 2> RescalingFactors_two(uint64_t timestep, Scalar deltaT) override{
            Scalar exp_thermo_fac = exp(-Scalar(1.0 / 2.0) * m_state.xi * deltaT);
            Scalar exp_thermo_fac_rot = exp(-m_state.xi_rot * deltaT / Scalar(2.0));
            return { exp_thermo_fac, exp_thermo_fac_rot };
        }

        void advanceThermostat(uint64_t timestep, hoomd::Scalar deltaT, bool aniso = true) override{
    ...
    public:
        BussiThermostat(std::shared_ptr<ParticleGroup> group,
                        std::shared_ptr<ComputeThermo> thermo,
                        std::shared_ptr<Variant> T,
                        uint16_t seed) : Thermostat(group, thermo, T), m_seed(seed) {}
