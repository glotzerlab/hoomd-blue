...
class pair_max_rcut_tests (unittest.TestCase):
    ...
    def test_nlist_subscribe(self):
        ...
        self.assertAlmostEqual(3.0, hoomd_script.context.current.neighbor_list.r_cut.get_pair('A','A'));
        self.assertAlmostEqual(2.5, hoomd_script.context.current.neighbor_list.r_cut.get_pair('A','B'));
        self.assertAlmostEqual(3.1, hoomd_script.context.current.neighbor_list.r_cut.get_pair('B','B'));

        # update a pair coefficient, and check
        lj.pair_coeff.set('A','B', r_cut = 5.0)
        hoomd_script.context.current.neighbor_list.update_rcut()
        self.assertAlmostEqual(5.0, hoomd_script.context.current.neighbor_list.r_cut.get_pair('B','A'))
        ...
        self.assertAlmostEqual(3.0, hoomd_script.context.current.neighbor_list.r_cut.get_pair('A','A'));
        self.assertAlmostEqual(5.0, hoomd_script.context.current.neighbor_list.r_cut.get_pair('B','A'));
        self.assertAlmostEqual(5.1, hoomd_script.context.current.neighbor_list.r_cut.get_pair('B','B'));
    ...
    def test_multi_nlist_subscribe(self):
        ...
        self.assertAlmostEqual(2.0, hoomd_script.context.current.neighbor_list.r_cut.get_pair('B','A'));
        self.assertAlmostEqual(5.1, hoomd_script.context.current.neighbor_list.r_cut.get_pair('B','B'));
