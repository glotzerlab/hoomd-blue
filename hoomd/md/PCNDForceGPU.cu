...
/*! \file PCNDForceGPU.cu
    \brief Defines GPU kernel code for calculating the Lennard-Jones pair forces. Used by PCNDForceComputeGPU.
*/
...
//! Kernel for calculating PCND Lennard-Jones forces
/*! This kernel is called to calculate the Lennard-Jones forces on all N particles for the PCND model potential.
...
__global__ void gpu_compute_pcnd_forces_kernel(const unsigned int group_size,
		                                Scalar4* d_force,
                                                Scalar* d_virial,
...
/*! \param d_force Device memory to write computed forces
    ...
    This is just a driver for calcPCNDForces_kernel, see the documentation for it for more information.
*/
hipError_t gpu_compute_pcnd_forces(const unsigned int group_size,
    ...
    // run the kernel
        hipLaunchKernelGGL((gpu_compute_pcnd_forces_kernel),
		           dim3(grid),
			   dim3(threads),
                           0,
			   0,
