...
struct meshtriangle_args_t
    ...
    meshtriangle_args_t(Scalar4* _d_force,
        ...
        : d_force(_d_force), d_virial(_d_virial), virial_pitch(_virial_pitch), N(_N), n_max(_n_max),
          ...
          d_head_list(_d_head_list), d_rcutsq(_d_rcutsq), size_neigh_list(_size_neigh_list),
          ntypes(_ntypes), block_size(_block_size), compute_virial(_compute_virial),
          devprop(_devprop) { };
...
__global__ void
gpu_compute_mesh_triangle_triangles_force_kernel(Scalar4* d_force,
                                                 Scalar* d_virial,
    ...
    Scalar* s_rcutsq = (Scalar*)(&s_data[ntypes * sizeof(typename evaluator::param_type)]);
    ...
    if (enable_shared_cache)
        ...
        unsigned int param_size = ntypes * sizeof(typename evaluator::param_type) / sizeof(int);
    ...
    if (active)
        ...
        for (int neigh_idx = threadIdx.x % tpp; neigh_idx < Nsize0; neigh_idx += tpp)
            ...
            for (int triangle_idx = 0; triangle_idx < n_triangles; triangle_idx++)
                ...
                Scalar normal_norm
                    = fast::rsqrt(normal_dir.x * normal_dir.x + normal_dir.y * normal_dir.y
                                  + normal_dir.z * normal_dir.z);
    ...
    static void launch(const meshtriangle_args_t& pair_args,
        ...
        if (tpp == threads_per_particle)
            ...
            size_t param_shared_bytes
                = (sizeof(Scalar) + sizeof(typename evaluator::param_type)) * pair_args.ntypes;

            unsigned int max_block_size;
            max_block_size = get_max_block_size(
                gpu_compute_mesh_triangle_triangles_force_kernel<evaluator,
                                                                 compute_virial,
                                                                 tpp,
                                                                 true>);
...
gpu_compute_mesh_triangle_triangles_force(const kernel::meshtriangle_args_t& meshtriangle_args,
    ...
    if (meshtriangle_args.compute_virial)
        MeshTriangleForceComputeKernel<evaluator, 1, gpu_pair_force_max_tpp>::launch(
            meshtriangle_args,
            threads_per_particle,
            d_params,
            tlist,
        ...
        MeshTriangleForceComputeKernel<evaluator, 0, gpu_pair_force_max_tpp>::launch(
            meshtriangle_args,
            threads_per_particle,
            d_params,
            tlist,
...
__global__ void
gpu_compute_mesh_triangle_particles_force_kernel(Scalar4* d_force,
                                                 Scalar* d_virial,
                                                 const size_t virial_pitch,
                                                 const unsigned int N,
                                                 const Scalar4* d_pos,
                            ...
                            for (unsigned int kkk = kk + 1; kkk < counter; kkk++)
                                ...
                                for (unsigned int jjj_tri = 0; jjj_tri < Njjj_tri; jjj_tri++)
                                    ...
                                    if (tri_idx == tri_idz)
                                        ...
                                        Scalar3 pos_a
                                            = make_scalar3(postypea.x, postypea.y, postypea.z);
                                        ...
                                        Scalar3 pos_b
                                            = make_scalar3(postypeb.x, postypeb.y, postypeb.z);

                                        Scalar4 postypec = __ldg(d_pos + cj);
                                        Scalar3 pos_c
                                            = make_scalar3(postypec.x, postypec.y, postypec.z);
                                        ...
                                        Scalar normal_norm
                                            = fast::rsqrt(dot(normal_dir, normal_dir));
                                        ...
                                        bool evaluated
                                            = eval.evalForceAndEnergy(force_divr, bond_eng, false);
...
gpu_compute_mesh_triangle_particles_force(const kernel::meshtriangle_args_t& meshtriangle_args,
    ...
    if (enable_shared_cache)
        ...
        if (meshtriangle_args.compute_virial)
            hipLaunchKernelGGL(
                (gpu_compute_mesh_triangle_particles_force_kernel<evaluator, 1, false>),
                grid,
                threads,
                shared_bytes,
                0,
            ...
            hipLaunchKernelGGL(
                (gpu_compute_mesh_triangle_particles_force_kernel<evaluator, 0, false>),
                grid,
                threads,
                shared_bytes,
                0,
