...
void ConstraintEllipsoid::update(uint64_t timestep)
    ...
    if (m_prof)
        m_prof->push("ConstraintEllipsoid");
...
void ConstraintEllipsoid::validate()
    ...
    if (m_P.x + m_rx > hi.x || m_P.x - m_rx < lo.x || m_P.y + m_ry > hi.y || m_P.y - m_ry < lo.y
        || m_P.z + m_rz > hi.z || m_P.z - m_rz < lo.z)
        {
        m_exec_conf->msg->warning() << "constrain.ellipsoid: ellipsoid constraint is outside of "
                                       "the box. Constrained particle positions may be incorrect"
                                    << endl;
        }

    if (m_rx == 0 || m_ry == 0 || m_rz == 0)
        {
        m_exec_conf->msg->warning() << "constrain.ellipsoid: one of the ellipsoid dimensions is 0. "
    ...
    for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
        ...
        if (dist > Scalar(1.0))
            ...
            m_exec_conf->msg->error()
                << "constrain.ellipsoid: Particle " << h_tag.data[j] << " is more than 1 unit of"
        ...
        if (h_body.data[j] != NO_BODY)
            {
            m_exec_conf->msg->error()
                << "constrain.ellipsoid: Particle " << h_tag.data[j] << " belongs to a rigid body"
...
void export_ConstraintEllipsoid(py::module& m)
    {
    py::class_<ConstraintEllipsoid, Updater, std::shared_ptr<ConstraintEllipsoid>>(
        m,
        "ConstraintEllipsoid")
