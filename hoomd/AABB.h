...
#if !defined(____HIPCC____) && defined(__SSE__)
#include <immintrin.h>
#endif

#if defined(__SSE__) && !defined(____HIPCC____)
...
#if defined(__AVX__) && !defined(____HIPCC____)
...
struct __attribute__((visibility("default"))) AABB
    {
    #if defined(__AVX__) && !defined(SINGLE_PRECISION) && !defined(____HIPCC____) && 0
    __m256d lower_v;    //!< Lower left corner (AVX data type)
    __m256d upper_v;   //!< Upper left corner (AVX data type)

    #elif defined(__SSE__) && defined(SINGLE_PRECISION) && !defined(____HIPCC____)
