        ...
        if (type_of_file == 1) {
            ...
            for (i = 0; i < nr; i++) {
                ...
                for (j = 1; j < m_ntypes; j++) {
                    h_rho.data[types[type] * m_ntypes * nr + j * nr + i].w = h_rho.data[types[type] * m_ntypes * nr + i].w;
                }
            }
        }
    }
 ...
 */
void EAMForceCompute::interpolation(int num_all, int num_per, Scalar delta,
