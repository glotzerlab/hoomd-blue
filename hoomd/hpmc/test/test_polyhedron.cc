...
UP_TEST( construction )
    ...
    UPP_ASSERT_EQUAL(a.data.verts.N, data.verts.N);
    for (unsigned int i = 0; i < data.verts.N; i++)
        {
    ...
    UPP_ASSERT_EQUAL(a.data.n_faces, data.n_faces);
    for (unsigned int i = 0; i < data.n_faces; i++)
        {
        UPP_ASSERT_EQUAL(a.data.face_offs[i], data.face_offs[i]);
        unsigned int offs = a.data.face_offs[i];
        for (unsigned int j = offs; j < a.data.face_offs[i+1]; ++j)
            UPP_ASSERT_EQUAL(a.data.face_verts[j], data.face_verts[j]);
        }

    UPP_ASSERT_EQUAL(a.data.face_offs[data.n_faces], data.face_offs[data.n_faces]);
...
UP_TEST( overlap_octahedron_no_rot )
    ...
    poly3d_data data;
    data.verts.sweep_radius = 0.0f;
    UPP_ASSERT(MAX_POLY3D_VERTS >= 6);
    data.verts.N = 6;
    UPP_ASSERT(MAX_POLY3D_FACES >= 8);
    data.n_faces = 8;
    ...
    UPP_ASSERT(MAX_POLY3D_FACE_VERTS >= 3);
    data.face_offs[0] = 0;
...
UP_TEST( overlap_cube_no_rot )
    ...
    UPP_ASSERT(MAX_POLY3D_FACES >= 6);
    data.n_faces = 6;
    UPP_ASSERT(MAX_POLY3D_FACE_VERTS >= 4);
    data.face_offs[0] = 0;
...
UP_TEST( overlap_cube_rot3 )
    ...
    r_ij =  vec3<Scalar>(0.9,-0.9,0.0);
    UPP_ASSERT(test_overlap(r_ij,a,b,err_count)); // this and only this test failed :-(
    UPP_ASSERT(test_overlap(-r_ij,b,a,err_count));
