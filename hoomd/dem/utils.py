...
def _polygonNormal(vertices):
    ...
    return -_normalize(
        np.cross(vertices[1] - vertices[0], vertices[0] - vertices[-1]))
...
def spheroArea(vertices, radius=1., factor=1.):
    ...
    delta = [
        (x2 - x1, y2 - y1) for ((x1, y1), (x2, y2)) in zip(vertices, shifted)
    ]
    lastDelta = [delta[-1]] + delta[:-1]
    thetas = [np.arctan2(y, x) for (x, y) in delta]
    dthetas = [(theta2 - theta1) % (2 * np.pi)
               for (theta1, theta2) in zip([thetas[-1]] + thetas[:-1], thetas)]
    ...
    sphereContribution = (sum([theta % np.pi for theta in dthetas
                               ])) / 2. * radius**2
...
def rmax(vertices, radius=0., factor=1.):
    ...
    """
    return (np.sqrt(np.max(np.sum(np.asarray(vertices) * vertices, axis=1)))
            + radius) * factor
...
def _fanTriangles(vertices, faces=None):
    ...
    if faces is None:
        if len(vertices) < 3:
            return
        for tri in ((vertices[0], verti, vertj)
                    for (verti, vertj) in zip(vertices[1:], vertices[2:])):
            yield tri
    else:
        for face in faces:
...
def massProperties(vertices, faces=None, factor=1.):
    ...
    if len(vertices[0]) == 2:
        ...
        a_s = [(x1 * y2 - x2 * y1)
               for ((x1, y1), (x2, y2)) in zip(vertices, shifted)]
