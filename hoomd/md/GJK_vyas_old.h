...
// Uncomment the line below to enable the recursive GJK solution for
// overlapping shapes.
...
DEVICE inline void johnson_algorithm(vec3<Scalar>* W, bool* W_used, Scalar* lambdas, Scalar deltas[][ndim+1], unsigned int added_element, bool& use_last)
    {
    use_last = false;
...
template <unsigned int ndim>
DEVICE inline Scalar gjk(vec3<Scalar> *verts1, unsigned int N1, vec3<Scalar> *verts2, unsigned int N2, vec3<Scalar> &v, vec3<Scalar> &a, vec3<Scalar> &b, bool& success)
    {
    clock_t begin = clock();
    // At any point only a subset of W is in use (identified by W_used), but
    // the total possible is capped at ndim+1 because that is the largest
    ...
    bool W_used[max_num_points];
    unsigned int indices1[max_num_points];
    unsigned int indices2[max_num_points];
    ...
    while (!close_enough)
        ...
        if (!close_enough)
            ...
            bool use_last(false);
            johnson_algorithm<ndim>(W, W_used, lambdas, deltas, new_index, use_last);
            if (use_last)
                {
                break;
                }
    ...
    clock_t end = clock();
    Scalar elapsed_secs = Scalar(end - begin) / CLOCKS_PER_SEC;
    return elapsed_secs;
    }

template <unsigned int ndim>
DEVICE inline void gjk_inline(vec3<Scalar> &pos1, vec3<Scalar> &pos2, vec3<Scalar> *verts1, unsigned int N1, vec3<Scalar> *verts2, unsigned int N2, vec3<Scalar> &v1, vec3<Scalar> &v2, vec3<Scalar> &a, vec3<Scalar> &b, bool& success, bool& overlap, bool in_recursion=false)
    {
    ...
    if (overlap && !in_recursion)
        ...
        gjk_inline<ndim>(pos1, pos2, &pos1, 1, verts2, N2, v1, v1, a1, b1, success1, overlap1, true);
        gjk_inline<ndim>(pos1, pos2, verts1, N1, &pos2, 1, v2, v2, a2, b2, success2, overlap2, true);
        a = a2;
        b = b1;
#ifndef NVCC
